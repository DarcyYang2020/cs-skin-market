...
- **三大原则与工作流**：见 `references/project-principles.md`（算法改动必须先数据回测再改码；回测起点 2025-11-02；熊市侧重 14d 胜率）
- **关键决策记录**：见 `references/decision-log.md`（样本排除、价格锚、D 方案、恐慌共振过滤、大盘 v5.2 等历史决策）
- **定价锚（最新）**：悠悠有品 DOM 价最高优先级，fallback（Buff/C5）只补 K 线不覆盖价格；报告/快照/批量扫描全链路用悠悠锚
- **专家意见流程**：先分析组合方案 → 用户确认 → 数据测试 → 再改码
