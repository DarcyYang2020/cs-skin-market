...
async def analyze_fresh(item, good_id, exact_name, *, db_item_id=None, apply_anchor=True,
                        hard_error_no_kline=False, volumes=None, volumes_from_bars=False,
    ...
    参数：
      ...
      volumes         历史日成交量（去量后恒 0）
      volumes_from_bars  True 时 volumes 取 daily_bars 的 bar.volume（watchlist 历史行为）
    ...
    if hasattr(analysis, 'aux') and analysis.aux:
        analysis.aux.turnover_rate = 0.0
        analysis.aux.mean_volume_7d = 0
