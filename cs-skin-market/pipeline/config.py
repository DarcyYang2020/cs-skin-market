...
# 复验触发（满足任一即启动全参数重验）:
#   1) ~260 天新数据积累完成（约 2027-04-25）;
#   2) buy 信号连续 2 个月 14d 胜率跌破 70%;
#   3) 每月例行检查: 14d>=80%、30d>=55% 则维持冻结不动。
...
PARAM_FREEZE = {
    ...
    "triggers": ["~260 天新数据积累完成（约 2027-04-25）", "buy 信号连续 2 月 14d 胜率跌破 70%", "每月检查 14d>=80% / 30d>=55% 则不动"],
