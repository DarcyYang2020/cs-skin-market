...
ITEM_EXPECTANCY_STATS_CLEAN_CUR = {
    ...
    # caveat: CLEAN-CUR: 230 signals, v2-T9, panic family share 35.2% (97.5% of panic concentrated in 2026-05 single-event cluster), in_sale NULL + 0-value gap backfilled via csQAQ period=1095
    # display keys match action_label (panic/deep_value/accumulate); events = +/-3d clusters.
    # win30/avg30 are n30 subsets; ci14 = Wilson 95% interval.
...
BASELINE_LEDGER = {
    "HIST-FULL": {
        ...
        "role": "历史全窗口基准（冻结归档，不可复现），C 通道监测主口径",
    },
    "CLEAN-CUR": {
        ...
        "signals": 230,
        "caveat": "csQAQ period=1095 回补 in_sale NULL 与 2026-02~04 断档 0 值；当前引擎 v2-T9；panic 族占 35.2%，其中 97.5% 集中于 2026-05 单事件簇；较 NULL-only 中间基线(297)摘除 84 条伪供给收缩信号，仅展示参考不作监测告警。",
        "role": "当前引擎回填后干净窗口展示参考",
    },
