...
# CLEAN-CUR = 当前引擎·回调段干净基线（v2-T7，150 信号，data/_exp_v2t7_win_replay.json）
# 两套数字必须挂标签展示，禁止裸数字；本块由 references/sync_expectancy_config.py 自动生成（勿手改）。
ITEM_EXPECTANCY_STATS_CLEAN_CUR = {
    # auto-generated by references/sync_expectancy_config.py; do not hand-edit.
    # baseline: CLEAN-CUR | replay: C:\Users\81572\Desktop\codex\cs-model\cs-skin-market\data\_exp_v2t7_win_replay.json | signals: 150 | net 2%
    # caveat: CLEAN-CUR: 150 signals, v2-T7, panic single-event share 55.3%, missing 2026-02~04 bull segment
    # display keys match action_label (panic/deep_value/accumulate); events = +/-3d clusters.
    # win30/avg30 are n30 subsets; ci14 = Wilson 95% interval.
    ...
    # 吸筹/回补组（供给收缩 + 深度回调 + 基础分批）：深度回调低吸(33) + 供给收缩吸筹(10) + 基础分批(4) 全量（自动生成）
    "accumulate": {
        "label": "吸筹/回补组（供给收缩 + 深度回调 + 基础分批）",
        "n": 47,
        "events": 10,
        "win14": 61.7, "avg14": 20.79, "ci14_lo": 47.4, "ci14_hi": 74.2,
        "win30": 50.0, "avg30": 18.41,  # n30=46
    },
}
...
BASELINE_LEDGER = {
    ...
    "CLEAN-CUR": {
        ...
        "engine": "v2-T7",
        "replay": "data/_exp_v2t7_win_replay.json",
        "signals": 150,
        "caveat": "干净但 panic 单事件占 55.3%、缺 2026-02~04 牛市段；只作展示参考，不作监测告警。",
        "role": "当前引擎干净窗口展示参考",
    },
    "deprecated_intermediates": {
        "290": "DECISION-4 对齐生产口径后的中间 buy 数，非基线；已废弃，以 HIST-FULL=317 / CLEAN-CUR=150 为准。",
        "140": "DECISION-6 对齐重放翻转后的中间 buy 数，非基线；已废弃，以 CLEAN-CUR=150 为准。",
        "163": "v2-T6 matched-window 干净回放中间信号数（含 below_floor 泄漏），非基线；已废弃。",
        "149": "v2-T7 重放估算中间数（163-14），后被 150 信号 CLEAN-CUR 正式产物替代；已废弃。",
    },
}
# ============================================================
...
ENGINE_VERSION = "v2-T7"  # v2-T6 + 流动性地板 below_floor 禁后置升级（2026-08-14）；对应 PARAM_REGIME.param_history 台账
