...
# CLEAN-CUR = 当前引擎·回调段干净基线（v2-T8，data/_exp_v2t8_win_replay.json）
# 两套数字必须挂标签展示，禁止裸数字；本块由 references/sync_expectancy_config.py 自动生成（勿手改）。
ITEM_EXPECTANCY_STATS_CLEAN_CUR = {
    # auto-generated by references/sync_expectancy_config.py; do not hand-edit.
    # baseline: CLEAN-CUR | replay: C:\Users\81572\Desktop\codex\cs-model\cs-skin-market\data\_exp_v2t8_win_replay.json | signals: 149 | net 2%
    # caveat: CLEAN-CUR: 149 signals, v2-T8, panic single-event share 57.0%, missing 2026-02~04 bull segment
    # display keys match action_label (panic/deep_value/accumulate); events = +/-3d clusters.
    # win30/avg30 are n30 subsets; ci14 = Wilson 95% interval.
    # 恐慌族：恐慌退潮(43) + 恐慌共振(42) 全量（自动生成）
    "panic": {
        "label": "恐慌族",
        "n": 85,
        "events": 2,
        "win14": 90.6, "avg14": 31.51, "ci14_lo": 82.5, "ci14_hi": 95.2,
        "win30": 82.4, "avg30": 24.88,  # n30=85
    },
    # 深值企稳：深值企稳(18) 全量（自动生成）
    "deep_value": {
        "label": "深值企稳",
        "n": 18,
        "events": 5,
        "win14": 50.0, "avg14": 14.75, "ci14_lo": 29.0, "ci14_hi": 71.0,
        "win30": 61.5, "avg30": 66.67,  # n30=13
    },
    # 吸筹/回补组（供给收缩 + 深度回调 + 基础分批）：深度回调低吸(32) + 供给收缩吸筹(10) + 基础分批(4) 全量（自动生成）
    "accumulate": {
        "label": "吸筹/回补组（供给收缩 + 深度回调 + 基础分批）",
        "n": 46,
        "events": 10,
        "win14": 63.0, "avg14": 21.29, "ci14_lo": 48.6, "ci14_hi": 75.5,
        "win30": 51.1, "avg30": 19.03,  # n30=45
    },
}
...
BASELINE_LEDGER = {
    "HIST-FULL": {
        ...
        "signals": 317,
        "caveat": "含约 50% 缺失深度信号（2026-02~04 牛市段污染）；覆盖完整牛熊周期，但存在缺失深度旁路。",
        "role": "历史全窗口基准（冻结归档，不可复现），C 通道监测主口径",
    },
    "CLEAN-CUR": {
        "label": "CLEAN-CUR 当前引擎·回调段基线",
        "engine": "v2-T8",
        "replay": "data/_exp_v2t8_win_replay.json",
        "signals": 149,
        "caveat": "干净但 panic 单事件占 57.0%、缺 2026-02~04 牛市段；只作展示参考，不作监测告警。",
        "role": "当前引擎干净窗口展示参考",
    },
    "deprecated_intermediates": {
        "290": "DECISION-4 对齐生产口径后的中间 buy 数，非基线；已废弃，以 HIST-FULL=317 / CLEAN-CUR=149 为准。",
        "140": "DECISION-6 对齐重放翻转后的中间 buy 数，非基线；已废弃，以 CLEAN-CUR=149 为准。",
        "163": "v2-T6 matched-window 干净回放中间信号数（含 below_floor 泄漏），非基线；已废弃。",
        "150": "v2-T7 泄漏基线（含 1 条 below_floor 吸筹旁路，产物已归档为 data/_exp_v2t7_win_replay_deprecated_20260814.json），已被 v2-T8 真基线 149 替代；非基线，已废弃。",
        "149": "v2-T8 真基线信号数（CLEAN-CUR），不再是中间数；见 CLEAN-CUR。",
    },
}
# ============================================================
...
ENGINE_VERSION = "v2-T8"  # v2-T7 + 吸筹周期 watch->buy 尊重流动性过滤（2026-08-14）；对应 PARAM_REGIME.param_history 台账
