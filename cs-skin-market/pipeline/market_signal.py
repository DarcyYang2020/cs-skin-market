...
_ACTION = {
    "P恐慌深跌": ("buy_zone", "抄底区",
                "恐慌深跌后 V 型反弹概率历史最高，分批建仓窗口"
                "（样本=2 个黑天鹅事件窗：五合一 2025-10/炼金 2026-05，不可外推普通深跌；"
                "共振抄底>独跌 14d 差 +24.8pp——P5）"),
    "S1牛市上行": ("hold_zone", "持多区", "慢牛主升——涨得慢但稳，回调低吸优于追高"
                                "（该时期引擎精选≈等权：14d +3.6 vs 等权 +5.8，无选股超额——P1 结构性结论）"),
    "S2牛市回调": ("pullback_buy", "回调买点区", "二波前夜——短平中正，60d 期望转正扩大"),
    "S3弱市阴跌": ("avoid", "空仓区", "规避——大盘全期限负期望，只保留精选腿"
                               "（大盘级裁定；独特性七形式除外：逆市强势品见单品报告状态行）"),
    "S4弱市反弹": ("trap", "反抽陷阱区", "反弹多为派发——不追反弹，新仓从严"
                               "（大盘级裁定；独特性七形式除外：逆市强势品见单品报告状态行）"),
}

# 证据窗标注（2026-08-17 主路径(3) R1）：P 期样本全部来自黑天鹅事件窗
_EVIDENCE_CAVEAT = {
    "P恐慌深跌": "该时期样本 45 天全部来自 2 个黑天鹅事件窗（五合一 2025-10 / 炼金 2026-05），"
               "前视数字不可外推到普通恐慌深跌",
}
...
def market_signal(conn=None):
    ...
    out = {
        ...
        "action_note": note,
        "period_forward": ev,
        "evidence_caveat": _EVIDENCE_CAVEAT.get(period, ""),
