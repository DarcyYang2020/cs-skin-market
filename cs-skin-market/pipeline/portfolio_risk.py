...
复验依据 data/b1_risk_validation_v2.json（去量引擎 v2 370 信号，references/b1_risk_backtest_v2.py）：
- cap0.8 强化: 全量 +193.30% / -9.39%（旧引擎 +54.6%/-15.3%）
- 熔断10% 失效（全量 0 触发）→ 语义降级为「信号质量劣化监测器」（监控非操作）
- dd5% 破坏性禁用；dd8% 压回撤但付 ~28pp 收益，仅监控
- 单票10% 硬上限二次证伪（-28~-16pp）→ 单票只做提示（30% 阈值），不做拒绝。
