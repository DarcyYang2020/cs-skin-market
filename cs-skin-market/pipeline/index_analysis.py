...
def analyze_cycle_sector_recommendation(cycle_phase: str, accumulation_prob: float = 0.0) -> dict:
    """Recommend sectors based on current market cycle phase.

    Returns ranked sector recommendations with rationale.
    """
    # Phase-specific sector preferences (CS2 market domain knowledge)
    PHASE_SECTOR_WEIGHTS = {
        "accumulation": {
            # Mainstream weapons lead recoveries, highest liquidity
            "main_weapon":   {"weight": 9, "rationale": "????????????????????"},
            "thousand_weapon": {"weight": 9, "rationale": "??/???????????????????"},
            "wk":            {"weight": 7, "rationale": "????Beta?????+????????"},
            "covert_weapon": {"weight": 6, "rationale": "????????????????????"},
            "knives":        {"weight": 5, "rationale": "?????????????????"},
            "gloves":        {"weight": 5, "rationale": "??????????????????"},
            "agent":         {"weight": 4, "rationale": "??????????????????"},
            "sticker":       {"weight": 2, "rationale": "???????????????"},
            "collection":    {"weight": 1, "rationale": "???????????????"},
        },
        "consolidation": {
            "agent":         {"weight": 8, "rationale": "???????????????"},
            "main_weapon":   {"weight": 7, "rationale": "????????????????"},
            "thousand_weapon": {"weight": 6, "rationale": "??????????????"},
            "knives":        {"weight": 5, "rationale": "???????????"},
            "gloves":        {"weight": 5, "rationale": "???????????"},
            "wk":            {"weight": 4, "rationale": "???????????"},
            "covert_weapon": {"weight": 4, "rationale": "??????"},
            "sticker":       {"weight": 3, "rationale": "????????"},
            "collection":    {"weight": 1, "rationale": "??"},
        },
        "markup": {
            "gloves":        {"weight": 9, "rationale": "????????Beta???????"},
            "knives":        {"weight": 9, "rationale": "???????????????"},
            "covert_weapon": {"weight": 8, "rationale": "???????????+??????"},
            "main_weapon":   {"weight": 7, "rationale": "?????????????"},
            "thousand_weapon": {"weight": 6, "rationale": "???????????"},
            "sticker":       {"weight": 5, "rationale": "?????????"},
            "wk":            {"weight": 5, "rationale": "????"},
            "agent":         {"weight": 4, "rationale": "??????????"},
            "collection":    {"weight": 2, "rationale": "?????????????"},
        },
        "distribution": {
            "main_weapon":   {"weight": 5, "rationale": "????????????,??????"},
            "thousand_weapon": {"weight": 4, "rationale": "??????????"},
            "agent":         {"weight": 3, "rationale": "?????"},
            "knives":        {"weight": 2, "rationale": "???????"},
            "gloves":        {"weight": 2, "rationale": "??????"},
            "covert_weapon": {"weight": 2, "rationale": "??????"},
            "wk":            {"weight": 1, "rationale": "?????"},
            "sticker":       {"weight": 1, "rationale": "?????"},
            "collection":    {"weight": 0, "rationale": "????"},
        },
    }

    weights = PHASE_SECTOR_WEIGHTS.get(cycle_phase, PHASE_SECTOR_WEIGHTS["consolidation"])

    # Load current sub-index data for momentum scoring
    try:
        from . import db as _db3
        conn = _db3.get_conn()
        raw = _db3.get_setting(conn, "cached_sub_indices", "[]")
        conn.close()
        import json
    ...
    # Score each sector: weight * phase_fit + momentum bonus
    recommendations = []
    for key, cfg in weights.items():
        si = sub_indices.get(key, {})
        change_7d = si.get("change_7d", 0) if si else 0

        # Base score from phase fit
        base = cfg["weight"] * 10

        # Momentum bonus: positive momentum adds, negative subtracts
        mom_bonus = min(15, max(-15, change_7d * 2))

        total = base + mom_bonus
        recommendations.append({
            "key": key,
            "name": si.get("name", key) if si else key,
            "score": round(total, 1),
    ...
    pl = phase_labels.get(cycle_phase, ("??", ""))
