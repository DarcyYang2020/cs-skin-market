...
| A-5 | ~~贴纸「观察桶·禁 buy」隔离标注~~ → 撤销（贴纸停采停扫，见 A-9） | — | — | A-9 | 撤销 |
...
### A-5 ~~贴纸「观察桶·禁 buy」隔离标注~~ —— 撤销（贴纸停采停扫）
- **撤销原因**：贴纸模块已决定停采停扫、退出发现榜（见 A-9），榜单不再混入贴纸，隔离标注失去意义。
- **替代动作**：A-9 落地后，discover 只保留主引擎综合榜；贴纸行不再渲染。
...
### A-9 贴纸模块停采停扫（数据/调度/展示层）—— 2026-08-13 已落地
- **目标**：回收 csQAQ 采集预算给主引擎；贴纸决策贡献为 0（S-2 证伪、W-3 未过 A2、观察桶禁 buy），只保留零采集静态研究资产。
- **内容**：
  - 停采：印花% 160 品统一加「活跃池淘汰:贴纸模块停采」标记（自选贴纸豁免），退出 run_daily_collect.py:166 每日 K 线；数据保留。
  - 停扫：webapp/main.py:1640 discover scope=sticker 关闭/移除，贴纸不进高分榜/热力图；双榜刷新收口为主引擎综合榜。
  - 冻结 alpha：run_daily_collect.py:401 W-3 吸筹跟踪 subprocess 摘除；STICKER-1 / W-3 / S 系列 A2 标 frozen；signal-family-registry.md 贴纸段标 frozen。
  - 保留静态资产：S-1 深历史快照 + 庄盘指纹块（analysis_service.py:868，读快照缓存）+ W-3 探索版产物 + first-principles-manipulation.md，不物理删除。
- **引擎侧任务**：无决策参数改动；仅数据层标记 + 调度摘除 + 展示层收口。
- **验收**：次日 18:00 全量采集不再采集贴纸；discover 无贴纸榜；W-3 tracker 不再每日运行；贴纸报告点开仍可看历史（零采集）。
- **预估**：1 天。
- **落地记录（2026-08-13）**：停采=159 个非自选/持仓贴纸标记 `贴纸模块停采`、自选贴纸 1 品豁免；discover 移除贴纸榜、scope 收口为 skin；每日 W-3 tracker subprocess 摘除；`items.notes` 排除逻辑落地。零引擎参数，历史数据未删。
...
### B-3 ~~贴纸吸筹正式 A2（W-3）~~ —— 取消/冻结
- **取消原因**：贴纸模块停采停扫、alpha 专项冻结（见 A-9）；W-3 探索版结论与 66 信号跟踪产物保留为静态研究资料，不推进正式 A2。
- **状态**：取消；重启条件 = 主引擎稳定 + csQAQ 采集预算宽松 + 用户明确重启贴纸专项。
...
- **与 roadmap 的关系**：A-1→J-2 配套；A-2→F-1 增强；A-3→F-7 轻量先行（monitor 消费端）；A-4→2026-08-12 口径审计延伸；A-5 撤销（贴纸停采停扫）；A-6→A1-4 门槛产品化；A-7→B1 提示层 UI；A-8→D-3 延伸；B-1 新（回放聚合）；B-2→F-2；B-3 取消（贴纸冻结）；A-9→贴纸停采停扫（新增）；B-4→F-7；B-5→F-4；B-6 新（提醒层评审）；B-7 新（研究）。
