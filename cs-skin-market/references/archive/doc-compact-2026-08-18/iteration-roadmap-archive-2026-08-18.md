# 迭代路线图 · 历史归档卷（2026-08-18 DOC-COMPACT-1）

> 本卷为 `references/iteration-roadmap.md` 的历史版本细节 + 新思路映射 + 批次/状态追踪 + 各技术方案（v1~v70 + BUY-1/cycle-refit/v3-enhance）。
> 主文件（保留 v71 主路径 + 活跃立项卡 CLEANUP-1/DISPLAY-1/DISPLAY-2/DOC-COMPACT-1 + 版本摘要表）见 `references/iteration-roadmap.md`。
...
- `TREND-EXIT-1`（2026-08-15 立项，EXIT 第三次尝试，登记待样本）：TREND 族专用确认机制——只对 TREND 族（若未来落地）验证「趋势买点破位止损（回撤超买入后低点/跌破 MA30）是否优于该族 hold21」。分族止损差异化假设（TREND 族破位=趋势判断错，恐慌族下跌=噪声，故只在 TREND 族验证、分族对照）。n 现实检查：验证段 25 信号/去簇 7 <15 → 样本不足；触发=未来牛市段 TREND 族去簇 ≥15 独立事件。红线=不改 hold21 全局、不碰现有四族退出。P1。
- `POOL-3`：自动淘汰无观察期。P1。
...
## v3 收益增强器技术方案（2026-08-15，外审方案基线 v3-engine-enhance-2026-08-15.md）

> 范围/架构/预注册/护栏基准 = `references/v3-engine-enhance-2026-08-15.md`（本文不复制粘贴，仅引用）。执行顺序：L0 → L1（先第一号）→ TREND-1 A2 → L2 → 组合层。全程在 replay_cycle_win.db 研究，零生产风险。

### L0 PRICE-GRAIN-1（地基，已就绪）
- 裁定：回放统一**日线价**（period=1095），生产实时采集维持十分钟末点。
- **已落地**：`replay_cycle_win.db` 已是日线价（96 品，120641 行，2023-06-01~2026-08-14）；唯一基线信号数 = **186**（拟合段 53 + 验证段 133）。无需额外半天。

### L1 第一号贪婪禁买实验（生死开关）
- 对象：2024-09~2025-05 牛市拟合段，`sent≤30` 时 TREND-1 回调买点（§2 预注册条件）的 14d/30d 胜率。
- 判据：≥60% → TREND-1 开豁免（或 sent≤30 仓位减半）；仍 ~0% → TREND-1 只能 sent>30 触发。
- **关键 caveat**：真实贪婪指数（macro_history.greedy_index）仅覆盖 2026-02-03 起，2024-09~2025-05 无真实值，`sent≤30` 只能用**价格近似情绪**（approx_sentiment，与真实值 60 天重叠 spearman 仅 +0.118 弱相关）。实验结论必须挂此 caveat，不得当真实贪婪结论。

### TREND-GRID-1 A2（牛市回调宽网格族，须过三件套）
- 预注册（组合版）：大盘 regime 门多头排列、单品 TH≥55(真)+站上 MA30、回调 5~20%（放宽自 8~15%）、3 日不新低、供给 7 日不扩张>5%、90 日分位≤75%（放宽自 60%）、豁免贪婪禁买→已裁定不豁免、单票 0.10、hold21。
- 第三对照组（已过）：多头门内无条件基线 41.1%/+0.98%；组合版超额 +2.87pp。
- A2 三件套：walk-forward（拟合 2023-06~2025-08 / 验证 2025-08~2026-08，四个小牛市分窗单报）+ 事件级去簇 + 置换检验。
- **判定门槛（量化，逐条打钩，第③条加重）**：① 验证段 14d net ≥ 旧引擎同段 +3pp（或 Calmar 口径）；② 2025-08~10 分窗 14d 胜率 ≥60%；③ **2026-02~03 分窗 TREND-GRID-1 信号 ≤2% 且净期望非负**；④ 组合 maxDD 不破 −20%；⑤ 事件级去簇后 n≥15 才判定。任何落地 bump ENGINE_VERSION + 全链路 sync。

### 收益增强器收口结论（2026-08-15）
