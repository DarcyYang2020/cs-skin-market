...
**占号簿（追加新条目前必查）**：
- 已用编号至 **HW**（2026-08-28 13:18）：`FA FB FC FD FE FF FG FH FI GI GJ GK GL GM GN GO GP GQ GR GS GT GU GV GW GX GY GZ HA HB HC HD HE HF HG HH HI HJ HK HL HM HN HO HP HQ HR HS HT HU HV HW`
- 当前指针：**HX**（下一个新条目用 HX，随后 HY/HZ...）
...
1. **mchg7/21/30 的 DSR=1.0 是 Q5 桥伪信号**：mchg 是**市场环境因子（截面常数）**——同一日全池同值，Q5 分档后 top 档=全池平均收益（市场 beta，非因子选择能力）。R1 判据早已按"条件因子"处理（截面 IC 无定义），**P6 的 Q5 收益桥对截面常数因子不适用** → mchg 三个"校正后仍成立"**不构成市场因子有 alpha 的证据**，登记降级。
