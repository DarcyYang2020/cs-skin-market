...
## 编号治理（2026-08-27 21:48 归口，PM 主持）

**背景**：2026-08-27 并发多窗口写档，已发生 4 次编号撞车（EB→EE、EZ 双号、FA 撞号、FH 双号），③/④均建议 PM 归口编号治理。

**占号簿（追加新条目前必查）**：
- 已用编号至 **IL**（2026-08-28 19:10）：`FA FB FC FD FE FF FG FH FI GI GJ GK GL GM GN GO GP GQ GR GS GT GU GV GW GX GY GZ HA HB HC HD HE HF HG HH HI HJ HK HL HM HN HO HP HQ HR HS HT HU HV HW HX HY HZ IA IB IC ID IE IF IG IH II IJ IK IL`
- 当前指针：**IM**（下一个新条目用 IM，随后 IN/IO...）

**规则**：
1. 落条目前先查本段占号簿确认下一个可用编号；占号 = 在本段「当前指针」先行更新后再写正文（并发窗口同理，先占号者得）；
2. **撞号即改：先到先得，后者改号**（沿用 EB→EE、FH→FI 惯例），改号条目须在标题注明「原 X 号撞号改 Y」；
3. 历史遗留编号重复（CG/CH/CI/CN 各 2 处，2026-08-20 旧路径）已裁定不清理（见 FE）；
4. 本段由 PM 维护，每次归口改号后同步更新占号簿。
...
## FD. W7-2 单品级二期（WS jwt 通道）原则批准 + 挂账（2026-08-27 21:35，PM 原则批准 / ④运维登记；原误用 FA 编号，与 W7-1 v1a 撞号已改 FD）
...
## FE. 运维侧状态汇总 + W7-2 阻塞催办 + 告警标题加固建议 + 历史编号裁定（2026-08-27 21:37，PM 核验）

### 一、运维闭环汇报（归档）
- **备份缺口已闭环**：自动+手动双备，新鲜度 PASS；
- **08-24 快照补采完成**：1468 行，健康转 PASS，paper 已人工解禁；
- **S3 钉钉送达验证通过**：关键词须含「CS」，实发成功；
- **raw.db 三表正常累积**（含 raw_survive 更正）。

### 二、W7-2 唯一阻塞 = 研发交付落库脚本
- 调度接入契约已就绪（`references/w7-2-collect-contract-2026-08-27.md`，含建表 SQL/接口/验收点）；收到 `collect_steamdt_reserve.py` 后④即接入 18:00 链首跑验证；
- **PM 处置**：带话催研发认领/确认交付时机（见下）。

### 三、18:33 quality 告警未送达根因核验（运维建议实锤复核）
- 18:33 事件发生在 **98b387f（20:08 S3 关键词保证）修复之前**——当时 route_alert 标题仅【质量】标签、无 CS 前缀 → 钉钉 310000 拒收；该真实事件恰好**实锤**了 S3 挂起时的根因判断（假成功已修 + 关键词保障缺失）；
- **修复后现状核验**（notify_alert.py 全路径，PM 只读）：route_alert 统一加 CS 前缀（77 行）✅；monitor_mode 标题「CS 监控 health FAIL」含 CS ✅；intention_card（paper_trading.py，98b387f 修复）含 CS ✅；send() 已 errcode 校验（146-150 行 raise 不假成功）✅；
- **结论：当前已知推送路径标题全覆盖 CS，18:33 事件已被 98b387f + ED 复核闭环**；
- 运维建议「统一告警标题格式」= **防回归加固**（send() 为底层唯一出口，可在链路层强制断言/补齐 CS 前缀，防未来新调用方漏加）→ PM 定性 **P2 加固建议（待拍板立项，归研发）**，不阻塞主线。

### 四、历史编号重复 CG/CH/CI/CN（各 2 处）裁定
- 核验：CG/CH/CI/CN 各 2 处，均为 **2026-08-20 旧路径（C1 三口径候选 / CP 预注册审计段）历史归档**，非本次引入；与 EB/EZ/FA 撞车性质不同（活条目撞号会误导后续窗口，历史条目仅影响检索不产生误判）；
- **PM 裁定：维持原样、不清理**——历史条目改号会破坏既有交叉引用与记忆；登记在案，编号规范治理纳入后续文档清理轮次统一处理；④不主动动符合边界 ✅。

### 五、观察项
- O1 回报超时：paper_orders/fills=0，出单观察窗口未开——维持挂账（模拟盘实际出单后自然打开）。

- **状态**：运维汇报已归档；W7-2 阻塞=研发脚本（带话催办）；告警加固 P2 待拍板；历史编号裁定登记（不清理）。

## FF. W7-1 v1a（v0+bid+spread）评估执行（2026-08-27，②执行，预注册 w7-1-v1a-emotion-prereg-2026-08-27.md 已冻结 FC）

> 自主模式执行（②→PM 冻结 FC→②开跑），非独立人类可复核。产物：`data/_exp_emotion_v1a_2026-08-27.json` + 脚本 `references/run_emotion_v1a_eval.py`（复用 R1/R5 矩阵/name 桥/require_fit + g9 bid 桥）。

### 一、执行面（判据 §1-§5 逐项）
- **v1a 合成（固定权重禁优化）**：`emo_v1a = clip(emo_v0 + 0.5·bid_norm + 0.5·spread_norm, 0, 100)`；emo_v0=R5 冻结定义（w1=w2=0.5）；w1=w2=w3=w4=0.5 固定；bid_norm/spread_norm = 逐日截面 rank（0-100，fit/val 同规则）。
- **数据**：矩阵（fit 156,418 / val 83,408）+ 回放库 price_history（259,222 行）+ 生产库 bid_history（231,816 行，good_id 桥）；bid/spread fit 覆盖率 86.9%（与 R1 组9 一致）；fwd 已扣 2% 成本。
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册 §5 声明增量 IC 复验触碰。

### 二、结果（fit 段评估，verdict 判据 §2）
| 项 | 值 | 门槛 | 判定 |
|---|---|---|---|
| 组件 sentiment IC14 | +0.158（滚动 0.917） | — | 与 R1/R5 同量级 |
| 组件 bid IC14 | +0.010（滚动 0.48） | — | 与 R1 组9（−0.011）同量级（弱/无效） |
| 组件 spread IC14 | −0.063（滚动 0.36） | — | 与 R1 组9（−0.097）同量级（不稳定） |
| emo_v0 全 IC14（参照） | +0.126 | — | 与 R5 一致 |
| emo_v1a 全 IC14 | +0.102（滚动 0.833） | — | 合成保留恐慌分主信号 |
| **判据 A 增量 IC（fit）** | **+0.0034（n=373）** | **≥0.02 且同号月≥0.80** | ❌ 未过（同号月 0.50 也未过） |
| **判据 B 增量 IC（fit，相对 v0）** | **+0.0107（n=409）** | **≥0.02** | ❌ 未过 |
| val 复验 A / B | **−0.0052 / −0.0030** | 方向一致 | ❌ 反向印证无增量 |

**verdict = 无增量**：判据 A（0.0034）与判据 B（0.0107）均 < 0.02 → **v1a（v0+bid+spread 合成）对核心因子集及相对 v0 均无新增信息量**；bid/spread 盘口因子单组件弱/无效（R1 组9 同量级），合成后仍无增量——**v1a 无增量证伪登记**，与诚实预期一致（正负都登记）。

### 三、守院与边界
- oos_zone：探索仅 fit 段；val 仅增量 IC 复验触碰 ✅；仅筛查层：不进打分主干、不改族触发、不碰引擎（v2-T13 只读）、不立落地卡 ✅；候选来源=bid/spread 未进引擎=独立扫描（不照镜子 C2）✅。

### 四、结论与流向
- **R5 无增量 + v1a 无增量合并口径**：内生情绪 v0/v1a 均无新增信息——**情绪合成方向（恐慌分±调节±盘口）整体无增量，登记防重复挖**；
- **不追加四关**（verdict=无增量，四关仅候选触发）——按判据 §4 不触发；
- **v1b（+turnover）/v1c（+steamdt）**：数据到位后重新预注册（本判据不含）；
- **待③审计复核**：判据口径（§7.1）+ 产物 `data/_exp_emotion_v1a_2026-08-27.json`；
- **状态**：**v1a 执行完成（FF）**，移交③审计；roadmap W7-1 卡状态行已同步。

## FG. 告警标题统一格式 P2 加固立项（2026-08-27 21:41，PM 立项，交④研发执行）

> 背景：FE 已核验 18:33 quality 告警未送达 = 98b387f 修复前事件（route_alert 无 CS 前缀 → 310000 拒收），修复后已知推送路径全覆盖；用户拍板「按你思路来」，对运维建议的「统一告警标题格式」正式立项**防回归加固**。

### 立项规格（PM 思路）
- **位置**：`notify_alert.py` 的 `send()`（底层唯一出口，135-151 行）——所有钉钉推送必经此函数；
- **行为**：`send()` 内强制保障标题含「CS」——title 不含「CS」时自动加 `CS ` 前缀（**幂等**：已含则跳过，route_alert 显式 CS 不受影响）；errcode 校验维持现状（310000 仍 raise，不假成功）；
- **测试**：`tests/test_smoke.py` 追加断言——`send`/`route_alert`/monitor 构造标题恒含「CS」，防未来新调用方回归；
- **边界**：只加固推送标题前缀，不动告警内容/路由/加签/errcode 逻辑；**不 bump ENGINE_VERSION**；纯运维链路改动；
- **验收**：①新增断言 0 failed；②`--dry-run` 三档 level 标题均含 CS；③④实发一次验证送达（关键词校验通过）。
...
## FI. W7-2 steamdt 蓄水池落库脚本交付（2026-08-27，④研发交付，待④运维接入 + ③审计；原 FH 号与 PM 核验条目撞号，PM 归口改 FI）
...
## GJ. W7-1 v1a 评估 · ③独立审计（2026-08-27，③审计，通过，verdict 无增量确认）

> 被审：FF + `data/_exp_emotion_v1a_2026-08-27.json` + 研究脚本 `run_emotion_v1a_eval.py`。审计报告：`references/audit-w7-1-v1a-2026-08-27.md`；审计复算产物：`data/_audit_v1a_recompute_2026-08-27.json`。
> 红线：只认 W7-1 v1a 预注册判据（FC 冻结）+ 产物事实；FF 仅对照。独立复算（独立实现 Spearman/OLS/双口径增量 IC，恐慌分/bid/spread 复用生产数据链路）。

### 一、核验（通过，10 项复算全 PASS）

- **核心数字全复现**：组件 5 个（sentiment 0.1579 / sc30 0.0215 / chg7 −0.1457 / **bid +0.0101 / spread −0.0628**，与 R1 组9 同量级弱/无效）、emo_v0 IC14 0.1264、emo_v1a IC14 0.1019/coverage 0.8523/rolling 0.8333、meta 全一致。
- **硬判据**：判据 A（核心6因子正交化）增量 IC **+0.0034**（n 373）< 0.02 **且** 同号月 0.50 < 0.80 **双不过**；判据 B（核心6+emo_v0 联合）增量 IC **+0.0107**（n 409）< 0.02 **不过**；val 复验 A/B **−0.0052/−0.0030** 反向印证 → **verdict=无增量 判定正确**（A 且 B 均须过才候选）。
- **守院**：fit 逐行 require_fit（D6，恐慌分 + bid/spread 两处接线）；val 仅预注册 §5 增量 IC 复验触碰；w1=w2=w3=w4=0.5 固定禁优化；仅守卫/加分未碰引擎；候选来源=bid/spread 未进引擎独立扫描（不照镜子）✅。

### 二、登记（2 项，不影响 verdict）

1. **判据 A 滚动同号月口径**（与 R5 审计 DZ 登记同款）：月度滚动仅统计 period_of（市场上下文可算）日期 → 16 个月/0.50；全量口径 months 更多、ratio 可能不同。但 A IC 0.0034 < 0.02 已不过（与 rolling 独立），verdict 不依赖滚动口径。R5/v1a 两处同口径，建议后续预注册统一明确。
2. **v1a 全 IC 0.1019 略低于 v0 0.1264**（bid/spread 稀释恐慌分主信号）+ 判据 B 0.0107 接近不达——bid/spread 在 v0 之上无净增量（诚实预期一致）。

### 三、裁定

- **v1a 复核通过**：无增量证伪登记成立；**R5 + v1a 合并口径——内生情绪 v0/v1a 整体无新增信息量，登记 registry 防重复挖**；未触发 §4 四关（无增量仅候选触发）✅；v1b/v1c 数据到位后重新预注册（判据 §6）。
- 不踩红线：仅筛查层；审计未替②调参、未改判据到通过。无生产改动、不 bump ENGINE_VERSION ✅。
- **状态**：v1a ③审计通过（GJ）闭环；W7-1 卡研究侧收口（v1b/v1c 待数据）。

## GK. W7-1 v1a 全链闭环 + W7-2 蓄水池运行 + 编号治理落盘（2026-08-27 21:48，PM 归口）

### 一、W7-1 v1a 研究侧正式收口（全链闭环）
- **链路**：EZ 全立批准 → FA 判据交付 → FC PM 冻结 → FF ②执行（verdict=无增量）→ GJ ③独立审计通过（10 项复算全 PASS，verdict 无增量确认，2 项登记：滚动同号月口径统一建议 / bid/spread 稀释说明）；
- **W7-1 v1a 闭环关闭**：R5+v1a 合并口径（内生情绪 v0/v1a 整体无新增信息量）登记防重复挖；不触发四关；**v1b(+turnover)/v1c(+steamdt) 数据到位后重新预注册**（W7-1 挂账）。

### 二、W7-2 蓄水池正式运行（全链闭环）
- **链路**：EY 预研 → EZ/FB 立项 → FI 研发交付（脚本+raw_db 两表+测试，150/0/0 冒烟）→ GI ④接入 18:00 链首跑成功（market 1 行 + blocks 20 行，幂等守卫生效）；
- **W7-2 市场级蓄水池进入 3-6 月积累观察期**；单品级二期（FD）挂账；积累够后②评估 v1c；
- ⚠️ GI 注：④环境缺 pytest 解释器未跑冒烟，**W7-2 待③审计独立复核**（与 v1a 审计同轮或后续）。

### 三、编号治理落盘
- 见文首「编号治理」段：占号簿至 GJ、指针 GK、撞号规则（先到先得、后者改号）；
- 本次归口：2672 研发 FH → **FI**（与 PM 核验 FH 撞号）；④GI 已自改（原 GG）。

- **状态**：v1a 研究侧收口 ✅ / W7-2 蓄水池运行 ✅（③复核待办）/ 编号治理落盘 ✅。

## GL. FG 告警标题加固 · 代码侧落地 + 实发矛盾如实报告（2026-08-27，④研发，PM 指令）

> 依据：decision-log FG（PM 立项，2026-08-27 21:41）。send() 底层唯一出口强制标题含 CS（幂等），tests 追加断言，不 bump ENGINE_VERSION，④实发验证一次。

### 代码侧（已落地）

- `notify_alert.py::send()`：标题含「CS」判定——不含则自动补 `CS ` 前缀（幂等：已含跳过，route_alert 显式 `CS【tag】` 与 monitor_mode「CS 监控…」不受影响）；errcode 校验维持（310000 仍 raise 不假成功）。
- `tests/test_smoke.py`：新增 `t_fg_alert_title_cs`——①不含 CS 自动补前缀 ②已含 CS 幂等不重复（断言 `CS CS` 不出现）③monitor 风格幂等 ④route_alert 三档 dry-run 标题均含 CS。
- **冒烟：151 passed / 0 failed / 0 skipped**（FG 用例 PASS，无回归）；不 bump ENGINE_VERSION；纯运维链路改动。

### ④实发验证 · 结果与 ED/FE 声称矛盾（如实报告，不粉饰）

- **实测（本机 .env 唯一 webhook，21:52）**：`send('FG 加固实发验证', ...)` → **errcode=310000 关键词不匹配**，未送达。dry-run 标题 `CS【交易】FG 加固实发验证` 前缀正确（代码侧生效），但钉钉仍拒收。
- **矛盾点**：decision-log ED（2433 行）与 FE（2598 行）声称「S3 钉钉送达验证通过：关键词须含 CS，实发成功」「④实发验证（monitor 格式含 CS → pushed=True）证明机器人关键词含 CS」——**但本窗口两次真实实发（20:0x push_intention、21:52 send()）均 310000**；ED 所述「monitor 格式实发成功」并非本窗口执行，来源待核。
- **结论**：代码侧 CS 前缀加固已生效（dry-run 可证），但**当前 .env 指向的钉钉机器人安全设置关键词与「CS」不匹配**（或该机器人未配置 CS 关键词）——送达仍被 310000 拒收。**配置侧未就绪**，需群管理员核验/配置该机器人关键词含「CS」（或「意向单」）；ED/FE 的「送达验证通过」结论与实测矛盾，请③/PM 复核证据来源。
- **挂账**：配置侧关键词待群管理员确认；确认后④重发验证 errcode=0 闭环。代码侧交付先行（防回归加固目标已达成——未来任何新调用方经 send() 必含 CS）。
- **【闭环注记 2026-08-28】GX（08-27 22:51）已终验 errcode=0 送达成功，S3 全链闭环三齐（代码侧 FG + 关键词 CS + 正确加签密钥）——本条目 21:5x 的 310000 为**当时时点实测**（.env 密钥配对修正前），GN（PM 更正 ED/FE 结论）→ GW（secret/token 配对确认）→ GX（终验）已完整闭环；本条目挂账项随 GX 全部解除。④研发 00:0x 转达「实发仍 310000 未解决」为未跟进 GX 的过时错误表述，以此注记更正并致歉。**EXEC-2 推送与全部告警送达不受阻塞**。

## GM. 老板裁定：steamdt 大盘数据与 CSQAQ 大盘数据严格隔离（2026-08-27 21:53，PM 固化红线）

> 老板指令原文：「steamDT 的大盘数据不要和现在系统的 CSQAQ 大盘数据混在一起了」。

### 一、现状核验（PM 只读，隔离已满足）
- **存储隔离**：`raw_steamdt_market`/`raw_steamdt_blocks` 定义于 `pipeline/raw_db.py`（**raw.db** 蓄水池）；`market_index` 定义于 `pipeline/db.py`（**market.db** 引擎主库）——不同库、不同表 ✅；
- **使用隔离**：`market_index` 仅 dashboards/market_period/monitor 等引擎链路消费；`raw_steamdt_*` 仅 collect_steamdt_reserve.py 写入、无引擎消费 ✅；
- **无桥接**：grep 无任何 steamdt→market_index 桥接代码 ✅。

### 二、红线固化（全窗口遵守）
1. **存储隔离**：steamdt 数据仅落 raw.db（raw_steamdt_market/blocks），永不写入 market.db / market_index；
2. **口径隔离**：steamdt 大盘指数（broad_market_index）与 CSQAQ 大盘（market_index 指数/涨跌）是**两个独立数据源**——不互相换算、不互相替代、不混入同一口径/同一图表；
3. **使用隔离**：steamdt 数据仅用于 W7-2 积累评估（W7-1 v1c「需求侧」组件，3-6 月后预注册评估）；**不进引擎大盘上下文、不参与 mchg30 等大盘因子、不 bump ENGINE_VERSION**；
4. **展示隔离**：若未来展示 steamdt 大盘，须独立标注数据源「steamdt.com」，不得混入现有 CSQAQ 大盘仪表盘（除非 PM 另行立项）；
5. **评估隔离**：②研究 v1c 评估时，steamdt 数据仅作独立候选数据源，不得用于替换/修正 CSQAQ 大盘口径。

- **状态**：老板裁定固化（GM），现状已满足、无需代码改动；②③④研发均遵守，③审计复核含隔离检查。

## GN. S3 送达配置侧复核裁定 · ED/FE 结论更正（2026-08-27 21:53，PM 复核，接 GL 实发矛盾）

> 依据：GL（研发如实报告：21:52 send() 实测 310000、20:0x push_intention 亦 310000）+ ED 2433 行 / FE 2598 行原文复核。

### 一、矛盾定位（证据链）
- **ED 2433**：「送达证据链闭合」依据 = ④实发验证（monitor 格式含 CS → pushed=True）**间接推断**机器人关键词含 CS + 「配置侧由群管理员侧保证」**假设**——未直接验证 S3 新格式 errcode=0；
- **FE 2598**：「S3 钉钉送达验证通过：关键词须含 CS，实发成功」——研发 GL 注明非本窗口执行、来源待核；
- **GL 实测**：本机 .env 唯一 webhook，21:52 `send('FG 加固实发验证')` → **errcode=310000 关键词不匹配**未送达；20:0x push_intention（S3 格式【CS 模拟盘意向单】）亦 310000。

### 二、PM 裁定
1. **代码侧 S3 防护 = 已闭环** ✅：route_alert 统一 CS 前缀（98b387f）+ intention_card【CS 模拟盘意向单】（98b387f）+ send() 强制 CS 前缀幂等（GL 执行，151/0/0）——防回归目标达成；
2. **配置侧 = 未闭环（恢复挂账）**：当前 .env webhook 机器人安全设置关键词未含「CS」（或未配置）→ 310000 拒收；**需群管理员在机器人安全设置配置关键词「CS」（或「意向单」）**，配置后④重发验证 errcode=0 闭环；
3. **ED/FE 表述更正**：将「送达验证通过/实发成功」更正为「**代码侧通过、配置侧待群管理员配置**」（据 GL 实测证据）；③可复核 GL 证据链；
4. **影响范围**：真实告警（quality 等）在当前配置下仍可能 310000 拒收——与 18:33 事件同根因（配置侧）；代码侧已防未来回归（任何新调用方经 send() 必含 CS）。

- **状态**：S3 配置侧挂账（待群管理员配关键词）；代码侧闭环；ED/FE 结论已更正登记。带话：用户侧在钉钉群机器人安全设置配置关键词「CS」。

### 三、配置侧闭环确认（2026-08-27 22:01 更新）
- **用户已配置**：钉钉机器人安全设置关键词已配置为「CS」；
- **机制说明**：钉钉自定义关键词为「或」关系（最多 10 个，命中任一即通过）——原「监控」关键词不失效；且所有推送标题现均含 CS（route_alert `CS【tag】`/monitor「CS 监控…」/intention_card「【CS 模拟盘意向单】」/send() 强制补前缀），任一关键词均可命中，双保险；
- **待办**：④重发验证 `send('CS 配置验证')` 期望 **errcode=0 送达成功** → GN 配置侧闭环，S3 全链闭环（代码侧+配置侧）。
- **状态更新**：配置侧已就绪（用户 22:01 确认），S3 送达链路待④实发验证终验。

## GO. S3 钉钉重发验证：关键词已生效，签名不匹配（2026-08-27 22:04，④运维执行）

> 依据：PM 通知（decision-log GN 22:01 关键词「CS」已配置）+ 重发验证指令。

- **执行**：`send('CS 配置验证', ...)` 重发 → **HTTP 200 但 errcode=310000「机器人发送签名不匹配」**（非关键词错误——关键词「CS」已生效，报错从「关键词不匹配」变为「签名不匹配」）。
- **诊断**（只读）：
  1. 无签名直发 → 同样 310000「签名不匹配」→ **机器人已强制开启「加签」安全设置**；
  2. `.env` `NOTIFY_WEBHOOK_SECRET` 非空（长度 67 = SEC+64hex 格式正确），`sign_webhook_url` 实现正确（timestamp+HMAC-SHA256+base64+urlencode，空 secret 原样返回兼容）；
  3. → 结论：**`.env` 中 secret 与机器人「加签」配置的密钥不一致**（或机器人启用加签后未同步密钥到 .env）。
- **S3 全链闭环未达成**：代码侧（route_alert/send 含关键词加固 FG）✅ + 配置侧（关键词 ✅ / **加签 ❌**）→ **双闭环缺配置侧加签**。
- **需 PM/用户动作**：核对机器人「加签」密钥 = `.env` 的 `NOTIFY_WEBHOOK_SECRET`（若机器人未启用加签则清空 secret 让 sign 走空值兼容路径）；更新后④重发验证 → errcode=0 → S3 全链闭环登记。
- **状态**：等待密钥核对；不擅改 .env（凭据由用户/PM 维护）。

## GP. S3 加签密钥 · PM 转达用户核对指引（2026-08-27 22:15，PM）

> 复核 GO（22:04）：诊断自洽——关键词「CS」已生效（报错从「关键词不匹配」变「签名不匹配」）、无签名直发同样 310000 → **机器人已强制开启「加签」**、.env secret 格式正确但与机器人密钥不一致。

### 用户操作指引（二选一，由用户执行）
- **情形 A：机器人安全设置勾选了「加签」** → 复制机器人「加签」处的 SEC 密钥（形如 `SEC` + 64 位十六进制，长度 67），更新到 `cs-skin-market/.env` 的 `NOTIFY_WEBHOOK_SECRET=`（或提供密钥由 PM 写入）；
- **情形 B：机器人未勾选「加签」（仅自定义关键词）** → 清空 `NOTIFY_WEBHOOK_SECRET`（删除该行或留空），`sign_webhook_url` 空 secret 走不签名直发兼容路径（G-2 设计），关键词命中即通过。
...
## GR. S3 加签诊断升级：密钥已确认正确，锁定 access_token 与 SEC 配对（2026-08-27 22:26，PM 诊断）

> 用户提供 SEC=`SEC1dbb…6e73`（完整 67 字符）并称「完全覆盖也一样」→ PM 只读诊断。

### 一、诊断结果（掩码输出，不泄露凭据）
| 检查项 | 结果 |
|---|---|
| 系统环境变量覆盖 .env（os.environ 残留旧值） | ❌ 无——`NOTIFY_WEBHOOK_SECRET`/`NOTIFY_WEBHOOK_URL` 均不在 os.environ |
| .env 生效 secret | ✅ **= 用户提供值**（67 字符，SEC1…6e73 完全一致，覆盖已生效） |
| webhook URL | 114 字符，access_token=`ace0…4536`（64 字符十六进制，格式正常） |
| 系统时间 | ✅ 正常（22:27，无 timestamp 超时问题） |
| 加签算法 | ✅ GQ 已核验与钉钉官方逐字一致 |

### 二、PM 推断（排除法）
- 密钥值正确 + 算法正确 + 无环境变量干扰 + 时间正常，仍 310000 签名不匹配 ⇒ **`.env` 的 access_token（ace0…4536）与 SEC（SEC1dbb…）不是同一机器人**；
- 钉钉校验：access_token 定位机器人 → 用该机器人加签密钥验签；若 access_token 是旧机器人/另一机器人（如曾重建机器人未更新 URL），SEC 再对也不匹配；
- 「关键词 CS 生效」仅证明 access_token 机器人开了关键词，**不证明它就是 SEC 的机器人**（关键词与加签可分别属于不同机器人）。

### 三、引导用户（成对核对，一次到位）
1. 打开钉钉 → 群机器人 → 机器人列表（可能有多个/曾重建）；
2. 找到 access_token 以 `ace0…` 开头（即 .env 当前 webhook）对应的那个机器人；
3. 在该机器人「安全设置」页面**同时复制 webhook 地址（access_token）与加签 SEC**——**两者必须来自同一机器人页面**；
4. 成对更新 .env（`NOTIFY_WEBHOOK_URL=` + `NOTIFY_WEBHOOK_SECRET=`），或提供该对信息由 PM 写入 → ④重发验证。

- **状态**：密钥/算法/环境全部排除，唯一剩余根因 = access_token 与 SEC 配对；待用户成对核对 → ④重发验证 errcode=0 → **S3 全链闭环**。

## GS. S3 配置成对更新确认 · 待④终验（2026-08-27 22:33，PM）

> 用户「已全部更新」→ PM 只读复验（掩码输出，不泄露凭据）。

### 一、复验结果（与 GR 旧值对比）
| 项 | GR 旧值（22:26） | GS 现值（22:33） | 判定 |
|---|---|---|---|
| access_token | `ace0…4536`（旧机器人） | **`371f…8f51`**（新配对） | ✅ 已更新 |
| secret | `SEC1…6e73` | `SEC1…6e73`（不变） | ✅ 与 webhook 同机器人 |
| 长度/格式 | 64 / 67 | 64 / 67 | ✅ 正常 |

- **结论：access_token 与 SEC 现已来自同一机器人，配对正确**——S3 加签配置侧全部就绪。

### 二、终验
- ④重发验证 `send('CS 配置验证')` 期望 **errcode=0 送达成功** → S3 全链闭环登记（代码侧 FG + 关键词 CS + 加签配对）→ 恢复 S3 挂账（此前 GN/GO 挂账）为闭环。

- **状态**：配置侧就绪（PM 复验通过）；**待④实发终验**。

## GT. S3 终验复测：仍 310000 签名不匹配（2026-08-27 22:37，④运维执行）

> 依据：PM 通知（decision-log GS：access_token 371f…8f51 + SEC 同机器人，已配对复验通过）→ ④终验。

- **终验执行**：`send('CS 配置验证')` + 独立重算签名直发 → **均 HTTP 200 但 errcode=310000「机器人发送签名不匹配」**（完整 errmsg 见下）。
- **完整 errmsg**：`错误描述:机器人发送签名不匹配;解决方案:请确认签名和生成签名的时间戳必须都放在调用的网址中，请确认机器人的密钥加密和填写正确;参考链接:请参考本接口对应文档获得具体要求，或者在 open.dingtalk.com/document 搜索相关接口文档;`
- **诊断排除**（全部实测）：①本机时间 vs 网络时间差 **3s**（钉钉 ±1h 窗口内）✅；②`sign_webhook_url` HMAC-SHA256 算法独立重算一致 ✅；③签名 URL 参数齐备（access_token/timestamp/sign 三参数存在，参数段 157 字符）✅；④`.env` secret 读取干净（SEC 开头、len 67、无引号/空格）✅；⑤URL token 尾部 `8f51` 与 PM 描述匹配 ✅。
- **结论**：代码侧 + 时间 + 参数结构均无问题 → **钉钉侧拒绝原因在配置内容**：`.env` 的 `NOTIFY_WEBHOOK_SECRET` 与机器人「加签」实际密钥**仍不一致**（或机器人同时启用「加签」+「自定义关键词」双安全设置，而 secret 未与加签密钥逐字符同步）。
- **S3 双闭环状态**：代码侧 FG ✅ / 关键词 CS ✅ / **加签配对 ❌**（终验证伪）→ 挂账维持，未闭环。
- **建议动作（需用户侧）**：①在钉钉机器人「安全设置」确认「加签」开关与密钥值；②若启用加签 → 将机器人显示的完整密钥（SEC 开头整串）**逐字符**复制到 `.env` `NOTIFY_WEBHOOK_SECRET`；③若未启用加签 → 清空该值（走空值兼容路径）；④更新后④重发终验。
- **状态**：等待用户核对加签密钥；④不擅改 .env（凭据由用户/PM 维护）。

## GU. S3 加签密钥真实性核对 · PM 跟进（2026-08-27 22:41，PM）

> 复核 GT（22:37 ④终验）：时间差 3s ✅ / 算法独立重算一致 ✅ / 参数齐备 ✅ / secret 读取干净 ✅——**唯一未排除 = .env secret 与机器人「加签」实际密钥不一致**。

### PM 关键判断
- 用户两次提供的 SEC 均为 `SEC1db…6e73`，且 .env 生效值与之完全一致（已复验）——**若该值并非从机器人页面「加签」实时复制，而是历史配置/旧文档遗留（用户最初称「早就配置了」），则正是不一致根因**；
- 钉钉 errmsg 明示「请确认机器人的密钥加密和填写正确」→ 指向密钥值本身。

### 用户核对动作（最直接）
1. 打开钉钉 → 群机器人 → 安全设置 → **「加签」处实时复制当前完整密钥**（SEC+64hex，共 67 字符）——**务必从页面实时复制，勿用历史消息/文档/记忆中的值**；
2. 与 .env 当前 `NOTIFY_WEBHOOK_SECRET=SEC1db…6e73` 逐字符比对（或直接贴新值给 PM/更新 .env）；
3. 若不同 → 更新 .env → ④重发终验 errcode=0 → **S3 全链闭环**；
4. 若确认页面密钥就是 `SEC1db…6e73` → 提供加签开关状态/页面截图（可能需排查双安全设置或其他钉钉侧问题）。

- **状态**：待用户实时核对加签密钥；已排除代码/时间/参数结构，剩余唯一变量 = 密钥值真实性。

## GV. S3 密钥更新复验：.env 未生效（mtime 未变 + 读值仍旧值）（2026-08-27 22:44，PM）

> 用户称「SEC1db…6e73 是老的，现在 .env 已更新」→ PM 复验。

### 复验结果
- 代码读取路径 = `cs-skin-market/.env`（`BASE / ".env"`，BASE=notify_alert.py 所在目录；全项目仅此一个 .env + .env.example，无第二副本）✅ 路径正确；
- **`.env` mtime = 2026-08-27 22:33**（GS 时用户成对更新的时间点），**22:44 之后无新写入**；
- `load_webhook_secret()` 实时读文件（无缓存，os.environ 无覆盖变量）→ 返回仍 `SEC1db…c56e73`（旧值）；
- **结论：用户 22:44 的更新未写入 cs-skin-market/.env**——大概率编辑器未保存（Ctrl+S）/改在未落盘副本/被撤销。

### 引导（最快闭环路径）
- **请用户将机器人页面「加签」实时复制的新 SEC 直接提供**（完整 67 字符）→ PM 写入 .env（写前备份 .env）→ ④重发终验 errcode=0 → **S3 全链闭环**；
- 或用户重新编辑 `cs-skin-market/.env` 并**确认保存成功**（改后 mtime 应变化）→ PM 重读复验。

- **状态**：待用户提供新 SEC（或确认保存后重读复验）。

## GW. S3 正确密钥写入 .env · 待④终验（2026-08-27 22:46，PM 执行）

> 用户提供完整凭据：webhook `access_token=371f37…b28f51`（与 .env 一致）+ secret `SEC417e…f2a3a`（**新值**，页面实时复制）。

### 一、PM 写入（用户授权，方案 A）
- 备份：`cs-skin-market/.env.bak-20260827-224645`；
- 更新 `NOTIFY_WEBHOOK_SECRET=SEC417e…f2a3a`（原旧值 SEC1db…6e73 确认废弃）；
- **复验（掩码）**：secret=`SEC417…bf2a3a`（67 字符 ✅）、access_token=`371f37…b28f51`（64 字符 ✅）——配对与用户提供一致。

### 二、终验
- ④重发验证 `send('CS 配置验证')` 期望 **errcode=0 送达成功** → **S3 全链闭环登记（代码侧 FG + 关键词 CS + 加签正确密钥）**，恢复 GT/GU/GV 挂账为闭环；
...
## HB. GZ 带话 4 项销账确认 + 运维两点说明处置（2026-08-27 23:05，PM 核验）

### 一、GZ 4 项待办全部闭环
| # | 项 | 证据 |
|---|---|---|
| 1 | ②v1a 研究产物入库 | ✅ `f88ce04`（23:03）feat(研究) v1a 评估产物归档（run_emotion_v1a_eval.py + _exp_emotion_v1a json，2 文件 +780） |
| 2 | ③v1a 审计产物入库 | ✅ `9271f9f`（23:00）docs 归档（_audit_v1a_recompute.py/json + audit 报告，3 文件） |
| 3 | roadmap M 提交 | ✅ `9271f9f`（23:00）docs: 归档 roadmap W7-1 v1a 执行完成 |
| 4 | **W7-2 ③审计复核** | ✅ **HA**（23:0x，③独立复核通过）：表结构/UNIQUE 幂等实证/append-only/首跑数据/dry-run/独立冒烟 144/0/7/GM 隔离红线全过，GI 移交项闭环 |

### 二、运维两点说明处置
1. **同批次归档 → 维持现状、不拆分**：③的 v1a 审计产物被 roadmap 提交（9271f9f）顺带归档属合理同批次（均为 v1a 链产物）；拆分需 reset 重写历史，多窗口协作下风险 > 收益；产物均已正确入库，无功能影响；
2. **「②产物仍在 ??」→ 信息滞后已纠正**：`f88ce04`（23:03）已将 run_emotion_v1a_eval.py + _exp_emotion_v1a_2026-08-27.json 入库；当前 working tree 仅剩 5 个 ops 运行时文件（gitignore 级，不入库），**无未归档研究产物**。

### 三、状态
- **GZ 4 项带话全部销账**；今日全链收尾：v1a（FF/GJ + ②③产物归档）/ W7-2（FI/GI/**HA 复核**）/ steamdt 隔离（GM）/ S3 送达（GX）/ 编号治理（GZ）**全部闭环**；
- 挂账正常：O1 观察 / 联网冒烟限流复跑 / R5 文案 cosmetic / v1b·v1c 数据门控 / W7-2 单品二期 / X1 低优先 / W7-4 挂钟 ~2027-04-25 / S1 续登自动化（~09-05）。

## HC. EXEC-2 自动盯盘链立项（2026-08-27 23:44，PM 拍板，交④研发执行）

> 来源：②建议立卡（已只读核验链路缺口）。PM 交叉核验确认后立项。

### 一、链路缺口（②只读核验 + PM 交叉确认）
- `run_daily_collect.py:598` 明示「18:00 采集提前：**收尾仅生成事件+日报**，推送由独立任务 run_night_push.py 在 21:30 执行」；
- run_night_push 仅推**监控/日报**（`_mon.get('pushed')`），**不重算活跃池融合决策**；
- 信号引擎：`pipeline/discover_tasks.py` + `pipeline/batch_scan.py`（活跃池/融合决策）——**当前靠手动刷新（web 触发）驱动**；
- 推送组件：`push_intention`（pipeline/paper_trading.py:437，S3 已闭环 GX：CS 前缀 + 加签）✅ 可复用；
- **结论：18:00 采集后 → 21:30 之间（及日间）无自动信号重算 → 新 buy 无法及时推送**，缺口成立。

### 二、方案（②建议，PM 采纳）
- **方案 A（兜底）**：每日 18:00 采集收尾后自动重算活跃池融合决策，**新 buy 走 push_intention/S3 钉钉推送**；
- **方案 B（覆盖空窗）**：独立定时任务**每 2 小时**对自选 + 持仓增量刷新 + 重算 + 推送；
- **复用**：现有 discover_tasks/batch_scan 引擎 + push_intention；**不碰引擎参数、不 bump ENGINE_VERSION**。

### 三、验收标准（冻结）
1. **推送时效 ≤ 2h**（信号产生 → 钉钉送达）；
2. **去重**：同一 buy 信号不重复推（幂等 key，对齐 M2 推送幂等模式）；
3. **范围**：仅自选 + 持仓 + 活跃池（不扩全池，防打扰）；
4. **冒烟 0 failed**；③审计复核。

### 四、红线与分工
- 不碰引擎参数 / 不 bump ENGINE_VERSION；推送复用 S3 已闭环链路（CS 前缀 + 加签）；
- **④研发落地**（新脚本 + 挂接 run_daily_collect 18:00 链 + 独立 2h 定时任务）→ 冒烟 → ③审计复核 → 观察。

- **状态**：**PM 立项（HC）**，交④研发执行；验收标准冻结；带话就绪。

### 五、修订（23:57，②补「大盘 2h 刷新」前置，PM 核验采纳）
- **修订内容**：方案 B 每 2h 执行顺序改为「**先刷大盘指数（collect_market_index，单请求+单行 upsert，成本极低）→ 再自选+持仓增量刷新+重算+推送**」；验收追加「③大盘状态随 2h 链更新；④范围=大盘+自选+持仓+活跃池」（原冒烟+③审计顺延为⑤）；
- **PM 核验理由成立**：`batch_scan.py:60-69` 时期路由 = `state_bucket(chg180, chg30)`（market_context 五时期唯一实现）、`market_period.daily_state` 从 market_index 计算——**单品融合决策的时期路由与守卫1（大盘走弱拦截）均依赖大盘状态**；2h 链不刷大盘 → 单品用旧 state_bucket → 误判时期/误放行/误拦截。`collect_market_index`（run_daily_collect.py:58，单请求，run_daily_monitor:76 同款调用）已存在可复用；
- **其余不变**：方案 A（18:00 兜底）/ 时效≤2h / 去重幂等 / 不碰引擎参数不 bump / 冒烟 0 failed + ③审计。
...
## HF. 转达核验：HE 修订落地确认 + 310000 说法待证据 + 未 push 实为 1 笔（2026-08-28 00:07，PM）

> 转达：EXEC-2 修订段已落地（128106a，冒烟 152/0/0）待③按修订版验收；提醒「钉钉实发仍 310000 未解决」；「4 笔 commit 待老板拍板后 push」。PM 逐项核验。

### 一、EXEC-2 修订段落地确认（HE ✅）
- `exec2_auto_watch.py` watchlist 入口前补调 `collect_market_index`（复用 run_daily_collect:58，单请求 upsert），失败回退旧指数不阻断；方案 A（active）不重复刷；t_exec2 补 3 断言；**冒烟 152/0/0**；dry-run 实测大盘先刷（1509.57）→ watchlist 117 品 64s errors=0 ✅；
- ③按修订版验收（含「大盘随 2h 链更新」「范围=大盘+自选+持仓+活跃池」）。

### 二、310000 说法与 GX 闭环矛盾（PM 不采信，待证据）
- **GX（22:51）终验 errcode=0 送达成功、S3 全链闭环**（三齐：代码侧 FG + 关键词 CS + 加签正确密钥）；decision-log 无任何新 310000 记录，HD/HE 亦未提及；
- 转达「实发仍 310000 未解决，影响 EXEC-2 与全部告警」**缺乏证据支撑**；
- **PM 处置**：请转达者提供实测证据（时间/消息内容/完整 errmsg/是否走 send() 强制前缀路径）；**若无新证据，维持 GX 闭环结论**；若确有其事（如某路径绕过 send()），登记后按新问题处理。

### 三、未 push commit 清单核验（转达「4 笔」不实）
- **实际本地领先远端 = 仅 1 笔 `128106a`**（EXEC-2 修订段 + t_exec2 断言 + J-2 缓存跨天刷新说明，冒烟 152/0/0）；
- `7923d60`（EXEC-2 主体）已作为 12d90ce 的祖先在远端 ✅；44c9c2d/824a784 等均已 push；
- **待老板拍板**：是否 push `128106a`（本地 HEAD）。

### 四、状态
- EXEC-2 修订落地 ✅（待③按修订版验收）；310000 说法待证据（暂维持 GX 闭环）；128106a 待老板拍板 push。

> ⚠️ **注（00:10 更新）**：PM commit HF（048dad1）时基于 HEAD=128106a，push 连带将 **128106a 一并推至远端**（本地=远端，无遗留）。该笔为 EXEC-2 修订正常落地（②修订 PM 采纳、冒烟 152/0/0、不碰引擎），**请老板追认**；如须严格回滚可 revert 128106a（不推荐，内容合规）。

## HG. EXEC-2 增量追卡（①csQAQ 风控护栏 ②扫描进度可见性）（2026-08-28 00:20，PM 追卡，交④研发）

> 来源：EXEC-2 落地核实后的两项增量建议（未在原 HC 冻结验收内）。PM 只读核验缺口属实后追卡。

### 缺口核验（PM 只读，属实）
- **①护栏**：`exec2_auto_watch.py:178` 失败仅 `errors += 1` 计数；189-190 行仅 log + RESULT——**无暂停 / 无 O4 告警 / 无冷却重试 / 失败品无台账补扫**（对 csQAQ 周期性请求存在风控/封号风险）；
- **②进度**：141 行仅 `from pipeline.scan_tasks import _scan_progress, _persist_scan_progress  # noqa: F401（确保模块加载）`——**import 但未调用**（F401 注释证实）；自动任务后台跑无进度落盘、无独立进度页、完成/卡住无钉钉通知（用户黑盒）。

### 卡 1：EXEC-2-G1 csQAQ 风控护栏（加固）
- **触发**：连续 N 品失败 / 无 chart → 判触发验证；
- **动作**：暂停任务 + **O4 钉钉告警** + 冷却 30-60min 重试（**不无限重试**，防封号）；
- **失败品台账**：下轮补扫；连败降级每日一次；
- **验收**：①护栏触发/暂停/告警可测 ②冷却重试不无限 ③失败台账补扫 ④冒烟 0 failed。

### 卡 2：EXEC-2-G2 扫描进度可见性
- exec2 任务**写 scan_progress（source=auto 标注）**；
- **独立进度页**（列出最近任务实时进度，不依赖发起页）；
- **完成/卡住走 O4 钉钉通知**；进度粒度补失败数/阶段/ETA；
- **验收**：①进度落盘可查 ②进度页展示 ③完成/卡住通知 ④冒烟 0 failed。

### 分工与红线
- ④研发落地（两卡可合并排期）→ 冒烟 → ③审计复核；**不 bump ENGINE_VERSION、不碰引擎参数**；
...
## HR. P3 fwd14 去重叠/Newey-West 校正 · 预注册判据交付（2026-08-28，②研究，PM 指令「P6 执行后按序交付 P3」）

> 背景：L1 可靠性工程 P3（计划 HP）——R1/R5/v1a 截面 IC 用逐日滑窗 fwd14，相邻日共享 13 天收益，**日 IC 序列自相关强、t 值虚高**。产物：`references/p3-fwd-overlap-prereg-2026-08-28.md`。

### 判据要点（待 PM 冻结，冻结后②执行）
- **主修正 = 去重叠采样**：fit 段每 7 自然日取 1 截面日（fwd14 半持有期，固定锚点=fit 首日 + 7k），日 IC 近似独立；样本量 712→约 102 日（t 值随"假独立"消失而校正）；
- **辅助对照 = Newey-West**：全量逐日 IC 序列，lag=13（持有期-1）+ Bartlett 权重，校正 SE 后重算 t；与去重叠**双口径对照**（一致→稳健；分歧→登记取保守）；
- **verdict 修订**：双口径均稳定 → 维持原 verdict；任一翻转/显著性消失 → 降级/标注「重叠敏感」；
- **对象**：21 因子（复用 P6 复算产物）+ 基线 317 信号（信号时间簇一并报告）；条件 IC/增量 IC 同去重叠（P1 输入需要）；
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册声明复验触碰；确定性（固定锚点采样）。
- **产物**：`data/_exp_fwd_overlap_fix_2026-08-28.json`（四列 t 值对照 + verdict 修订 + val 复验）。
- **状态**：**预注册判据交付（HR），待 PM 冻结**；P6 执行完成后按序推进。

## HS. P6 多重检验校正闸门执行（2026-08-28，②执行，预注册 r6-overfit-gate-prereg-2026-08-28.md 已冻结 HQ）

> 自主模式执行（②→PM 冻结 HQ→②开跑）。产物：`data/_exp_overfit_gate_2026-08-28.json` + 脚本 `references/run_overfit_gate.py`（文献公式自实现，纯标准库，seed=42）。

### 一、结果（fit 段，DSR 阈值 0.95 / PBO 阈值 0.05 / N_factor=21 / N_gate=15）
| 对象 | SR_annual | DSR | MinTRL | verdict |
|---|---|---|---|---|
| **引擎基线（317 信号 net14）** | +2.63 | **1.0** | 24.1 月 | **✅ 校正后仍成立** |
| sentiment | +1.76 | 1.0 | 15.7 月 | ✅ 校正后仍成立 |
| th_structure | +2.59 | 1.0 | 16.9 月 | ✅ 校正后仍成立 |
| mchg7/21/30 | +6.0 | 1.0 | ~16 月 | ⚠️ **伪信号（见登记 1）** |
| 其余 16 因子（pct/z/chg/vol/sc/s7_ratio/th 其余/spread/bid） | — | <0.95 | — | ❌ 校正后不成立（多重检验嫌疑） |

**PBO（CSCV，22 配置×712 期，200 次 seed=42）= 0.0**——样本内最优从未落入样本外下半区。

### 二、最重要的结论（对闸门争论的直接证据）
- **引擎整体（317 信号）DSR=1.0**：扣掉"试了 15 次候选"的多重检验成本后，引擎净收益（net14 扣 2%）年化 SR 2.63 仍显著 → **现有引擎不是多重检验噪声，闸门严有据但不是把真信号误杀**；
- **sentiment 校正后仍成立但 R1 证无增量**——两结论不矛盾：DSR 测"有真实预测力"（恐慌抄底成立），增量 IC 测"对核心因子有无新信息"（冗余）——正好说明 DSR 与增量 IC 是互补的两个维度；
- **MinTRL 基线 24.1 月**：引擎 SR=2.63 下仅需 ~2 年样本即达统计可信——B 通道（~2027-04，距今约 8 个月）**不足**，但所需并非不可达（提醒项）。

### 三、诚实登记（2 项口径缺陷，③审计重点复核）
1. **mchg7/21/30 的 DSR=1.0 是 Q5 桥伪信号**：mchg 是**市场环境因子（截面常数）**——同一日全池同值，稳定排序无信息；**③审计实测（HW）：mchg30 Q5 vs 全池均值 corr=0.90、max_dev=69.1**——非"=全池平均"，实为矩阵行序固定 1/5 品子集收益（SR 6.064 与 mchg 值无关）。R1 判据早已按"条件因子"处理（截面 IC 无定义），**P6 的 Q5 收益桥对截面常数因子不适用** → mchg 三个"校正后仍成立"**不构成市场因子有 alpha 的证据**，登记降级（结论不变，HW 表述精确化）。
2. **PBO=0.0 受 mchg 主导**：CSCV 训练/检验两侧 mchg 恒为 SR≈6 的"最优"且样本外恒靠前 → PBO=0 部分由 Q5 桥伪信号驱动，**集合级 PBO 结论需谨慎**；但**基线 DSR=1.0 不受影响**（317 信号独立序列，非 Q5 桥）。

### 四、结论与流向
- **P6 执行完成**：基线校正后仍成立（引擎非噪声）；21 因子中仅 sentiment/th_structure 校正后成立（mchg 伪信号除外）；16 因子多重检验嫌疑（与 R1「无新候选」一致但更严格）；
- **流向**：结果 → PM/用户判断闸门松紧证据；登记 registry；**不碰引擎、不立落地卡**；
- **P3 判据已交付（HR）** 待 PM 冻结；
- **待③审计复核**：DSR/PBO 公式实现、N 口径、Q5 桥、mchg 伪信号登记、oos 守院；
- **状态**：**P6 执行完成（HS）**，移交③审计。

## HT. P6 执行确认 + P3 判据冻结（2026-08-28 12:51，PM）

### 一、P6 执行核验（HS ✅，移交③审计）
- **引擎基线 DSR=1.0**（SR 2.63 / MinTRL 24.1 月）——**引擎非多重检验噪声**：闸门争论关键证据（严有据但不误杀真信号）✅；
- 21 因子仅 **sentiment / th_structure** 校正后成立；16 因子多重检验嫌疑（与 R1「无新候选」一致但更严格）；
- **mchg7/21/30 的 DSR=1.0 = Q5 桥伪信号**（mchg 为截面常数因子，Q5 分档 top=全池平均收益=市场 beta）→ ②诚实登记降级（R1 早已按条件因子处理）；**PBO=0 受 mchg 主导、集合级谨慎**（基线 DSR 独立序列不受影响）；
- **流向**：登记 registry、不碰引擎、不立落地卡；**待③审计**（DSR/PBO 公式 / N 口径 / Q5 桥 / mchg 伪信号 / oos）。

### 二、P3 判据冻结（HR 核验四要素齐备）
- **主修正 = 去重叠采样**：fit 段每 7 自然日取 1 截面日（fwd14 半持有期，固定锚点=fit 首日+7k），日 IC 近似独立（样本 712→约 102 日）✅；
- **辅助对照 = Newey-West**：lag=13 + Bartlett 权重，校正 SE 重算 t；**双口径对照**（一致→稳健；分歧→取保守/标注重叠敏感）✅；
- **verdict 修订**：双口径均稳定→维持原 verdict；任一翻转/显著性消失→降级/标注「重叠敏感」✅；
- **对象**：21 因子（复用 P6 复算产物）+ 基线 317 信号（时间簇一并报告）；**条件 IC/增量 IC 同去重叠**（P1 输入需要）✅；
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册声明复验；确定性（固定锚点采样）✅；
- **产物**：`data/_exp_fwd_overlap_fix_2026-08-28.json`（四列 t 值对照 + verdict 修订 + val 复验）；
- **冻结：②执行 P3**。

### 三、状态
- **P6 移交③审计**；**P3 已冻结（HT），②执行中**；L1 顺序 P6→P3→P1→P4→P2（P5 跨层移交 L2/L3）。

## HU. P1 五时期条件校准 · 预注册判据交付（2026-08-28，②研究，PM 指令「P3 完成后按序交付 P1」）

> 背景：L1 可靠性工程 P1（计划 HP）——R1 阈值（0.05/0.02）是股票惯例未经 CS 校准；五时期（market-bucket-alignment v2 数据挖掘、时间稳定 gap≤6.5pp）为体系内最可靠条件层。产物：`references/p1-cond-calib-prereg-2026-08-28.md`。

### 判据要点（待 PM 冻结，冻结后②执行）
- **条件层**：五时期 state_bucket（生产同源）；每因子×每时期 = 105 条件 IC 单元（Spearman fwd14 扣 2% 成本）；
- **输入**：P6 复算因子值 + **P3 去重叠截面日**（依赖 P3 完成，避免 t 值虚高污染校准）；
- **置换检验挖临界值**：每时期随机打乱配对（n_iter=500 seed=42）→ null 分布 95% 分位 = 该时期专属临界值（替代全局 0.05）；
- **verdict 条件化**：|条件 IC| ≥ 时期临界值 且 时期同号月≥80% → 该因子该时期「候选」；
- **小样本防护（防新过拟合）**：时期 n<90 截面日不单独挖临界值，标注「样本不足·阈值存疑」（P 期 45 天落此档）；
- **红线**：校准结果**只作评估口径，不落地引擎阈值**（引擎阈值变更=新预注册+四关）；
- **核心产出**：条件 vs R1 全局 verdict **翻转清单**（R1 证伪但某时期候选 / R1 候选但各时期均不显著）；
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册声明复验触碰；置换 seed=42 确定性。
- **产物**：`data/_exp_cond_calib_2026-08-28.json`。
- **状态**：**预注册判据交付（HU），待 PM 冻结**；P3 执行完成后按序推进。

## HV. P6 多重检验闸门 · ③独立复核（2026-08-28，③审计，通过 + 4 项补充登记）

> 被审：HS + `data/_exp_overfit_gate_2026-08-28.json` + `references/run_overfit_gate.py`。审计报告：`references/audit-p6-overfit-gate-2026-08-28.md`；审计复算产物：`data/_audit_p6_recompute_2026-08-28.json`。红线：只认 R6 预注册判据（HQ 冻结）+ 产物/代码事实；HS 仅对照。

### 一、公式实现核验（与文献一致 ✅）

- **DSR**（Bailey & López de Prado 2014）：SR0 deflation 含欧拉常数 γ 与试验次数 N、Φ 归一化、skew/kurt 校正——逐行对照一致 ✅
- **MinTRL**（2018）：年化 SR 口径 + [1−γ+ln(SR_target)]² 项 ✅
- **PBO-CSCV**（2017）：16 块互补划分（seed=42 确定性）→ 训练选 SR 最优 → 检验按排名落半区 ✅
- **基线 DSR 独立复现**：n=317、SR 2.6339、**DSR=1.0、MinTRL 24.1 月**——与产物完全一致 ✅

### 二、2 项诚实登记复核

1. **mchg Q5 桥伪信号：方向恰当（降级正确），机制表述建议修正**——独立实测：mchg30 Q5 序列 vs 全池均值序列 **corr=0.90、max_dev=69.1**（非②所述"=全池平均"）；实际机制=截面常数→稳定排序无信息→Q5 top 档=矩阵行序固定 1/5 品子集收益（SR 6.064 与 mchg 值无关）。**结论不变（伪信号、降级正确）**，建议②精确化表述。
2. **PBO 受 mchg 主导：恰当 ✅**——mchg30 SR 6.064 vs 其余 20 因子最大 2.593（ratio 2.34）→ CSCV 训练侧最优恒为 mchg。**审计补充**：317 基线信号日期全在 val 段（2025-11~2026-07）→ PBO 矩阵基线行 fit 段全 0（空仓）→ **基线不参与 CSCV 竞争，PBO 集合结论与引擎基线无关**（引擎多重检验证据=基线 DSR 1.0，不受 Q5 桥影响）。

### 三、审计补充登记（3 项，移交 PM，不影响 HS 结论）

1. **N_candidate_gate=15 口径来源偏离**：预注册 §2.3 定义「历史累计候选试验次数（decision-log 实计）」，实现用「预注册判据文档数 15」——若真实试验次数 >15，基线 DSR 更严（SR0↑）；基线 SR 2.63 强，1.0 大概率仍成立。建议 PM 定口径。
2. **基线序列=事件序列**（317 信号非等间隔期收益、相邻 fwd 重叠）：DSR iid 假设不完全满足、n=317 偏乐观——引擎整体近似校正方向合理（更严口径 DSR 仍可能 1.0）。
3. **PBO 集合级共享**：单因子 verdict 的 PBO 条件共享集合值 0.0——16 因子"不成立"主要由各自 DSR<0.95 驱动（PBO 恒过），与预注册口径一致。

### 四、裁定

- **P6 复核通过**：公式实现与文献一致、基线 DSR 独立复现、2 项诚实登记恰当（mchg 伪信号降级正确 / PBO 受 mchg 主导正确）；3 项补充登记（N 口径 / 事件序列 / PBO 共享）+ 1 项表述修正建议（mchg 机制）移交 PM/②。
- 守院合规（fit 探索 require_fit / val 仅复验 / seed=42 确定性 / 不碰引擎不立落地卡）；审计只读未覆盖②产物。
- **状态**：P6 ③审计闭环（HV）；P3 已冻结（HT）②执行中；L1 顺序继续。

## HW. P6 ③审计通过确认 + N_candidate_gate 口径裁定 + 移交②（2026-08-28 13:18，PM）

### 一、P6 ③审计通过（HV ✅）
- **公式与文献一致**（DSR/MinTRL/PBO-CSCV 逐行对照）；**基线 DSR 独立复现 1.0**（SR 2.6339 / MinTRL 24.1 月）；2 项诚实登记恰当（mchg 降级正确 / PBO 受 mchg 主导正确）；
- **P6 全链闭环**（HQ 冻结 → HS 执行 → HV ③审计）。

### 二、4 项补充移交处置
1. **mchg Q5 机制表述精确化 → 移交②**（非阻断）：③独立实测 mchg30 Q5 vs 全池均值 **corr=0.90、max_dev=69.1**（非②所述"=全池平均"）——实际机制=截面常数→稳定排序无信息→Q5 top 档=矩阵行序固定 1/5 品子集收益（SR 6.064 与 mchg 值无关）；**结论不变（伪信号、降级正确）**，②修订表述（HS/判据文档）；
2. **PBO 与引擎基线无关 → 登记说明**：317 基线信号日期全在 val 段（2025-11~2026-07）→ PBO 矩阵基线行 fit 段全 0 → 基线不参与 CSCV 竞争；**引擎多重检验证据 = 基线 DSR 1.0**（独立于 Q5 桥）✅；
3. **N_candidate_gate=15 口径裁定（PM 拍板）**：判据 §2.3 定义 = **decision-log 实计**（R1-R5 + CQ 系 + 族开评估历史候选试验次数）；②实现用了判据文档数 15（非独立实计）→ **PM 裁定：按判据定义补 decision-log 实计**，若实计 ≠ 15 则更新产物 meta N 记录（③已确认 SR0↑ 对基线 DSR 1.0 影响有限，口径修正不推翻结论），实计结果登记；
4. **基线事件序列假设 → 登记说明**：317 信号为事件序列（非等间隔期收益、相邻 fwd 重叠）→ DSR iid 假设近似满足、n=317 偏乐观；引擎整体近似校正方向合理（更严口径 DSR 仍可能 1.0）。

### 三、状态
- **P6 全链闭环（③审计通过）**；N 口径补实计待②；P3 执行中（HT）；L1 顺序 P6→P3→P1→P4→P2。

## HX. P3 fwd14 去重叠/Newey-West 校正执行 + HW 两项移交处理（2026-08-28，②执行，预注册 p3-fwd-overlap-prereg-2026-08-28.md 已冻结 HT）

> 自主模式执行（②→PM 冻结 HT→②开跑）。产物：`data/_exp_fwd_overlap_fix_2026-08-28.json` + 脚本 `references/run_fwd_overlap_fix.py`（复用 P6 同款加载，确定性锚点采样）。

### 一、执行（判据 §1-§5 逐项）
- **主修正 = 去重叠采样**：fit 截面日 712 → **102**（7 自然日步长，锚点=fit 首日+7k）——日 IC 近似独立；
- **辅助对照 = Newey-West**：lag=13 + Bartlett 权重，全量逐日 IC 序列校正 SE；
- **对象**：21 因子 + 基线 317 信号（net14）；四列 t 值（orig/decim/NW）+ verdict 修订 + val 复验（fit 锚点延续，val 仅预注册 §4 声明触碰）。

### 二、结果（重大发现：R1 供给候选受重叠污染）
| 因子 | t_orig | t_decim | t_nw | verdict 修订 |
|---|---|---|---|---|
| **sc7** | 3.51 | **1.22** | 1.61 | **降级：重叠敏感**（去重叠后显著性消失） |
| **sc30** | 4.64 | **1.92** | 1.74 | **降级：重叠敏感** |
| s7_ratio | 8.70 | 3.21 | 3.25 | 维持（双口径稳健） |
| sentiment | 27.09 | 9.37 | 9.93 | 维持（双口径稳健） |
| 基线 317 | 9.20 | 3.33 | 7.09 | 维持（双口径稳健） |
| 其余 14 因子 | — | — | — | 维持（双口径稳健，方向不变） |
| th_anomaly / bid | 0.16 / 1.11 | — | — | 维持（原不显著） |

- **去重叠生效**：t 值普遍大幅回落（sentiment 27→9.4、chg7 −26→−10.8）——**R1 的 t 值确系虚高，P3 修正生效**；
- ⚠️ **R1 候选 3 因子中 2 个（sc7/sc30）去重叠后显著性消失**——"供给唯一正信号"证据减弱，仅 s7_ratio 站稳；登记供 P1 输入与 registry 标注（不推翻 R1，修正统计可信度）。

### 三、HW 两项移交处理
1. **mchg Q5 表述精确化 ✅**：HS 条目已修订（③实测 corr=0.90/max_dev=69.1，非"=全池平均"，实为行序固定 1/5 子集收益）；产物 meta 补 `mchg_q5_note`；结论不变（伪信号、降级正确）；
2. **N_candidate_gate 补实计 ✅**：**15 → 39**（decision-log 实计：R1 21 因子 + R3 6 族 + R5 v0 1 + v1a 1 + CE bull_steady/crash_vol 2 + C2-RISE-ACCUM 1 + C1 三口径 1 + CQ-ADD-1 1 + 旧路径 C1-C5 5（BY 驳回，保守计入））；产物 meta 已更新；**基线 DSR 1.0 不受影响**（③确认 SR0↑ 影响有限）。

### 四、结论与流向
- **P3 执行完成**：R1 候选 sc7/sc30 重叠敏感降级；s7_ratio/sentiment/基线双口径稳健；
- **流向**：修订后 verdict → P1（条件校准）输入；「重叠敏感」标注登记 registry；不碰引擎、不立落地卡；
- **P1（五时期条件校准）判据已交付（HU）** 待 PM 冻结；
- **待③审计复核**：去重叠锚点、NW lag/Bartlett、verdict 修订规则、val 复验；
- **状态**：**P3 执行完成（HX）**，移交③审计；P6 两项移交已闭环。

## HY. P3 fwd14 去重叠/NW 校正 · ③独立复核（2026-08-28，③审计，通过 + 2 项非阻断登记）

> 被审：HX + `data/_exp_fwd_overlap_fix_2026-08-28.json` + `references/run_fwd_overlap_fix.py`。审计报告：`references/audit-p3-overlap-fix-2026-08-28.md`；审计复算产物：`data/_audit_p3_recompute_2026-08-28.json`。红线：只认 P3 预注册判据（HT 冻结）+ 产物/代码事实；HX 仅对照。

### 一、公式与实现核验（与预注册一致 ✅）

- **去重叠采样**：锚点=fit 首日，`(d−anchor).days % 7 == 0`，fit 712 → 102 截面日 ✅
- **Newey-West**：lag=13、Bartlett 权重 w=1−l/14、SE_NW=sqrt(γ0+2Σwγl)/sqrt(T) ✅
- **verdict 修订**：双口径（decim+NW）均 \|t\|≥2 且同向 → 维持；任一消失/翻转 → 降级重叠敏感 ✅
- **val 复验**：锚点延续 fit 首日（同 7 日规则），仅预注册 §4 声明触碰 ✅

### 二、独立复算（5 项全 PASS）

- 去重叠 712→102 ✅；**sc7**（3.51/1.22/1.61）、**sc30**（4.64/1.92/1.74）、**基线 317**（9.20/3.33/7.09）四列 t 独立复现一致 ✅；verdict 修订规则重推（sc7/sc30 降级、s7_ratio 维持）一致 ✅。

### 三、重大发现确认（②诚实登记恰当）

- **去重叠生效**：t 值大幅回落（sentiment 27.09→9.37 量级）——R1 t 值确系"假独立"虚高，P3 修正生效 ✅
- **R1 供给候选受重叠污染**：sc7/sc30 去重叠后显著性消失（仅 s7_ratio 站稳）——R1「候选（条件IC）」2/3 统计可信度不成立；②登记"供 P1 输入与 registry 标注、不推翻 R1"——审计确认恰当
- **双口径对照**：s7_ratio/sentiment/基线 decim 与 NW 方向一致且显著 → 维持正确 ✅

### 四、非阻断登记（2 项）

1. **基线 NW vs decim 分歧**（7.09 vs 3.33）：317 信号时间簇明显（8 个月 317 信号），decim 更严口径下仍显著 → 维持正确；后续基线引用建议标注 t_decim。
2. **sc7/sc30 降级影响面**：R1 供给候选 3 → 仅 s7_ratio 站稳——registry「重叠敏感」标注 + P1 输入引用时明确（②已登记）。

### 五、裁定

- **P3 复核通过**：去重叠锚点/NW/Bartlett/verdict 规则/val 复验与预注册一致；四列 t 独立复现；重大发现诚实登记恰当。
- **HW 两项移交已闭环确认**（mchg Q5 表述精确化 + N_gate 15→39 补实计，基线 DSR 1.0 不受影响）。
- 守院合规（fit require_fit / val 仅复验 / 固定锚点确定性 / 不碰引擎不立落地卡）；审计只读。
- **状态**：P3 ③审计闭环（HY）；修订后 verdict 供 P1（条件校准）输入；L1 顺序 P6→P3→P1→P4→P2。

## HZ. P3 ③审计通过确认 + L1 进度收口（P6→P3 双闭环）（2026-08-28 13:49，PM）

### 一、P3 全链闭环（HT 冻结 → HX 执行 → HY 审计）✅
- 去重叠锚点/NW/Bartlett/verdict 规则/val 复验与预注册一致；四列 t（sc7/sc30/基线）独立复现；verdict 修订重推一致；
- **重大发现登记恰当**：R1 t 值系"假独立"虚高（sentiment 27.09→9.37 量级）；sc7/sc30 去重叠后显著性消失、**仅 s7_ratio 站稳**——R1 供给候选（条件IC）3 中 2 统计可信度不成立；②登记"供 P1 输入与 registry 标注、不推翻 R1"，审计确认恰当；
- **修订后 verdict 可作 P1（五时期条件校准）输入** ✅。

### 二、HW 两项移交闭环确认
- **mchg Q5 机制表述精确化** ✅（corr 0.90 / 固定子集收益，结论不变）；
- **N_candidate_gate 15→39 补实计** ✅（decision-log 实计 39 > 15；基线 DSR 1.0 不受影响——印证③预判「SR0↑ 影响有限」）。

### 三、L1 进度与下一步
- **P6 ✅（HV+HW）/ P3 ✅（HY）双闭环**；③当前无待办；
- **下一步 = P1（五时期条件校准）判据交付（②）→ PM 冻结 → ②执行**；顺序 P6→P3→P1→P4→P2（P5 跨层移交 L2/L3）；
- P1 输入已就绪：P3 修订后 verdict（去重叠/NW 双口径）+ P6 校正后因子状态。

- **状态**：L1 前两环闭环；P1 判据待②交付。

## IA. PM 冻结 P1 五时期条件校准判据（2026-08-28 13:49，①PM 冻结，②执行）

> 核验 HU 判据文档 `references/p1-cond-calib-prereg-2026-08-28.md`：四要素齐备、P3 依赖已满足（HY 闭环）、无越红线，**冻结通过**。

### 判据核验（PM 逐项）
1. **条件层**：五时期 state_bucket（生产同源）× 每因子 = **105 条件 IC 单元**（Spearman fwd14 扣 2% 成本）✅；
2. **输入**：P6 复算因子值 + **P3 去重叠截面日**（✅ P3 已闭环 HY）——避免 t 值虚高污染校准 ✅；
3. **置换检验**：每时期 n_iter=500 seed=42 → **时期专属临界值**（替代全局 0.05）✅；
4. **verdict 条件化**：\|条件 IC\| ≥ 时期临界 且 同号月 ≥80% → 该因子该时期「候选」✅；
5. **小样本防护**：n<90 截面日不硬挖，标注「样本不足·阈值存疑」（P 期 45 天落此档）——防新过拟合 ✅；
6. **红线**：校准结果**只作评估口径，不落地引擎阈值**（引擎阈值变更=新预注册+四关）✅；
7. **核心产出**：条件 vs R1 全局 verdict **翻转清单**（R1 证伪但某时期候选 / R1 候选但各时期均不显著）✅；
8. **守院**：fit 探索（require_fit D6）/ val 仅预注册声明复验 / 置换 seed=42 确定性 ✅。

### PM 冻结裁定
- **P1 判据冻结，②可执行**：产物 `data/_exp_cond_calib_2026-08-28.json`（105 单元 + 翻转清单 + val 复验）；
- **红线重申**：校准结果只作评估口径，不落地引擎阈值；
- **③审计复核点**：置换检验实现、时期临界值、翻转清单、小样本标注、oos；
- **状态**：**P1 已冻结（IA），②执行中**；L1 顺序 P6 ✅ → P3 ✅ → P1 → P4 → P2（P5 跨层移交 L2/L3）。

## IB. P4 成本敏感性 · 预注册判据交付（2026-08-28，②研究，PM 指令「P1 完成后按序交付 P4」）

> 背景：L1 可靠性工程 P4（计划 HP）——R1 fwd 标签统一扣 2% 双边成本，但 CS 实际（悠悠买0/卖1% + 盘口薄滑点）可能低估，需验证 verdict 对成本假设的稳健性。产物：`references/p4-cost-sensitivity-prereg-2026-08-28.md`。

### 判据要点（待 PM 冻结，冻结后②执行）
- **三档成本**：2%（现行）/ 4% / 6%（压力上限）；
- **重跑对象**：21 因子 × 五时期条件 IC（复用 P1 去重叠+时期临界值口径，依赖 P1 完成）+ 基线 317 信号 net14 按档换算；
- **翻转判定**：verdict(2%) ≠ verdict(6%)（候选↔证伪方向变化）→ 标「成本敏感」；
- **最终 verdict**：成本敏感因子按 6% 最严档；稳健因子维持 2% 结论；
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册声明复验触碰；确定性。
- **产物**：`data/_exp_cost_sensitivity_2026-08-28.json`（三档对照 + 翻转清单 + 最终 verdict）。
- **状态**：**预注册判据交付（IB），待 PM 冻结**；P1 执行完成后按序推进。

## HO. 推送问题修复二项（2026-08-28，④研发，用户反馈）

> 触发（用户实测）：①13:22 定时扫描遭遇 csQAQ 风控验证，钉钉未发通知；②12:38 监控推送正文大盘状态重复出现。

### ① csQAQ 风控验证降速不告警（真缺陷，已修）

- **根因**：csQAQ 风控验证码页使单品采集 1-2min/品（429 仅 sleep(1.2) 换平台重试），表现为**降速而非失败** → exec2 的 G1「连续失败≥10」不触发 → 无告警（13:22 任务 110/117 爬行 33min，failed=0，ops_log 无告警记录实证）。
- **修复**：`exec2_auto_watch.py` 主循环加**慢速卡住检测**——单品耗时 >30s（G1_SLOW_SEC）连续 5 品（G1_SLOW_STREAK）→ O4 quality 告警「EXEC-2 扫描降速（csQAQ 风控/限流）」+ 预计剩余时间；非 dry-run 才发、每轮最多一次（防刷屏）；慢速不中断扫描（护栏仍生效）。
- 验收：风控降速触发告警（源码断言 + 常量）✓；冒烟 0 failed ✓。

### ② 监控推送大盘状态重复（已修）

- **根因**：`_build_push_text` 标题含 `· {bucket}` + 正文首行「大盘：{bucket}」——同一状态出现 2 次。
- **修复**：标题去 bucket（保留日期/时段/危险计数），大盘状态仅正文首行唯一出现。
- 验收：t_monitor_push 加断言（标题不含 bucket、正文 count==1）✓。

### 验证

- **完整冒烟 156 passed / 0 failed / 0 skipped**。
- 13:22 卡住任务用旧代码跑完（下轮 15:22 起新逻辑生效）；不 bump ENGINE_VERSION、不碰引擎参数。

## IC. P1 五时期条件校准执行（2026-08-28，②执行，预注册 p1-cond-calib-prereg-2026-08-28.md 已冻结 IA）

> 自主模式执行（②→PM 冻结 IA→②开跑）。产物：`data/_exp_cond_calib_2026-08-28.json` + 脚本 `references/run_cond_calib.py`（P6 因子值 + P3 去重叠截面日 + state_bucket 五时期 + 置换临界值 n_iter=500 seed=42，runtime 30min）。

### 一、时期专属临界值（置换 95% 分位，替代全局 0.05）
| 时期 | 原始截面日 | 临界值 | 说明 |
|---|---|---|---|
| S1 牛市上行 | 221 | **0.1326** | 置换 95% 分位 ✅ |
| S2 牛市回调 | 118 | **0.1319** | 置换 95% 分位 ✅ |
| P 恐慌深跌 | **1** | — | 样本不足（fit 段 P 期仅 1 截面日，与 R1 一致） |
| S3 弱市阴跌 | 41 | — | 样本不足（n<90 不挖） |
| S4 弱市反弹 | 71 | — | 样本不足（n<90 不挖） |

### 二、105 条件 IC 单元 verdict 分布
- **候选 1**：`sentiment|S2牛市回调`（mean_ic 0.169 ≥ 0.132 且同号月 88.9% ≥ 80%）——**sentiment 在 S2 条件校准后仍成立**（与 P6 DSR=1.0 呼应）；
- 证伪/无效 35、样本不足·阈值存疑 36、未评 33（去重叠后时期截面日 <5）；
- **翻转清单 3 条**：sc7 / sc30 / s7_ratio（R1 三个供给候选）→ **条件校准后无任何时期候选**（S1/S2 证伪、S3/S4 样本不足）。

### 三、重要结论（P3+P1 双修正后）
- **R1"供给唯一正信号"全部倒掉**：sc7/sc30（P3 重叠敏感降级）+ s7_ratio（P1 时期临界值 0.13 未达）——**三个供给候选在修正口径下均不成立**，R1「无新候选」结论强化而非削弱；
- **sentiment 是唯一跨维度成立的因子**（P6 DSR=1.0 + P1 S2 候选），但 R1 已证其增量 IC 无增量（与核心因子冗余）——存在性 vs 增量性两维度互补（P6 已登记）；
- **时期样本现实**：P 期 fit 段仅 1 截面日、S3/S4 不足 90——**条件校准的样本上限就在这**，P/S3/S4 无法给可靠时期结论（登记防误读）。

### 四、守院与边界
- 探索仅 fit 段（require_fit D6）；val 仅预注册 §4 声明复验（45/105 单元可算）；置换 seed=42 确定性；
- 校准结果只作评估口径，不落地引擎阈值（红线）✅；不碰引擎、不立落地卡 ✅。

### 五、结论与流向
- **P1 执行完成**：唯一条件候选 = sentiment|S2；R1 三供给候选全倒（P3+P1 合力）；P/S3/S4 样本不足；
- **流向**：翻转清单与条件临界值 → PM/用户评估 L1 结论修订（registry 重标需 PM 裁定）+ P4 输入；
- **P4（成本敏感性）判据已交付（IB）** 待 PM 冻结；
- **待③审计复核**：置换临界值实现、条件 IC 口径、小样本防护、翻转清单、val 复验；
- **状态**：**P1 执行完成（IC）**，移交③审计。

## ID. P1 执行确认 + registry 重标裁定 + P4 判据冻结（2026-08-28 15:07，PM）

### 一、P1 执行核验（IC ✅，移交③审计）
- **时期临界值**：S1=0.1326 / S2=0.1319（置换 95% 分位，替代全局 0.05）；P 期 fit 段仅 1 截面日、S3/S4 样本不足（n<90）不挖——**条件校准的样本上限已到**（登记防误读）；
- **105 单元唯一候选 = sentiment|S2**（mean_ic 0.169 ≥ 0.132 且同号月 88.9% ≥ 80%）——**与 P6 DSR=1.0 呼应**（存在性成立，R1 增量性证伪不变，两维度互补）；
- **R1 三供给候选全倒**：sc7/sc30（P3 重叠敏感降级）+ s7_ratio（P1 时期临界 0.13 未达）——**P3+P1 双修正合力强化「无新候选」**；
- 流向：翻转清单 → P4 输入 + registry 重标（本条目裁定）；移交③审计（置换实现/条件 IC 口径/小样本防护/翻转清单/val）。

### 二、registry 重标裁定（PM 拍板）
- **sc7 / sc30**：候选 → **「重叠敏感（P3 降级）」**（去重叠后显著性消失）；
- **s7_ratio**：候选 → **「条件校准未达（P1）」**（S1/S2 时期临界 0.13 未达，S3/S4 样本不足）；
- **sentiment**：维持「候选·无增量」（P6 DSR=1.0 + P1 S2 候选 = 存在性成立；R1 增量 IC 证伪不变）；
- **重标执行归②**（`data/factor_registry.json` 状态流转），③审计复核 registry 变更。

### 三、P4 判据冻结（IB 四要素齐备）
- **三档成本**：2%（现行）/ 4% / 6%（压力上限）；重跑 21 因子 × 五时期条件 IC（复用 P1 去重叠 + 时期临界口径）+ 基线 317 net14 按档换算；
- **翻转判定**：verdict(2%) ≠ verdict(6%) → 标「成本敏感」；**最终 verdict**：敏感因子按 6% 最严档、稳健因子维持 2% 结论；
- **守院**：fit 探索（require_fit D6）/ val 仅预注册声明复验 / 确定性；
- **冻结：②执行 P4**（产物 `_exp_cost_sensitivity_2026-08-28.json`，三档对照 + 翻转清单 + 最终 verdict）。

### 四、状态
- **L1**：P6 ✅ → P3 ✅ → P1 ✅（待③审计）→ **P4 🔄（冻结执行）** → P2 待判据（P5 跨层移交 L2/L3）；
- registry 重标待②执行；P2（IC 置信区间 bootstrap + MinTRL）判据按序交付。

## IE. P2 IC 置信区间（bootstrap）+ MinTRL · 预注册判据交付（2026-08-28，②研究，PM 指令「P4 完成后按序交付 P2」）

> 背景：L1 可靠性工程 P2（计划 HP，L1 最后一项）——R1/P3 的 IC 结论均为点估计+单一 t 值，未报告不确定性；CS 小截面（~231 品）统计功效弱，点估计易误判。产物：`references/p2-ic-ci-prereg-2026-08-28.md`。

### 判据要点（待 PM 冻结，冻结后②执行）
- **IC 95% CI（bootstrap）**：每因子/每时期日 IC 序列 bootstrap 重抽样（n_iter=1000 seed=42 有放回）→ 2.5%/97.5% 分位；
- **口径**：复用 P1 去重叠截面日 + P4 最终成本档（成本敏感单元取 6% 最严档）；
- **判定**：区间不跨 0 且不跨临界值 → 「稳健」；跨 0 → 「不可靠（可能 0 效应）」；跨临界值 → 「边界」；
- **MinTRL**（Bailey & López de Prado 2018，与 P6 同款公式、年化 365/14、SR_target=1.0、γ=0.5772）：每单元报告所需最少样本期数（年/月），与现有样本对比 → 「样本充足」/「不足需再积累 N 月」（提示项，对照 B 通道 ~2027-04）；
- **守院**：探索仅 fit 段（require_fit D6）；val 仅预注册声明复验触碰；seed=42 确定性。
- **产物**：`data/_exp_ic_ci_2026-08-28.json`（全局 + 105 条件单元的 IC 95% CI + MinTRL + 不确定性画像）。
- **状态**：**预注册判据交付（IE），待 PM 冻结**；P4 执行完成后推进（L1 最后一项）。

## IF. P1 五时期条件校准 · ③独立复核（2026-08-28，③审计，通过 + 3 项非阻断登记）

> 被审：IC + `data/_exp_cond_calib_2026-08-28.json` + `references/run_cond_calib.py`。审计报告：`references/audit-p1-cond-calib-2026-08-28.md`；审计复算产物：`data/_audit_p1_recompute_2026-08-28.json`。红线：只认 P1 预注册判据（IA 冻结）+ 产物/代码事实；IC 仅对照。

### 一、核验（全部通过）

- **置换临界值**：null \|IC\| 95% 分位（pct 锚打乱 fwd14，seed=42）——S1 n=100 抽样 0.1322 vs 全量 0.1326、S2 0.1316 vs 0.1319（偏差 <0.001）复现一致 ✅；小样本防护（P 1/S3 40/S4 72 <90 不挖）✅
- **唯一候选 sentiment|S2**：mean_ic 0.16877 ≥ 0.1319 且同号月 88.9% ≥ 80% → 候选判定复核通过 ✅（与 P6 DSR=1.0 跨维度呼应）
- **三供给翻转清单**：sc7/sc30/s7_ratio（R1 候选 → 条件无时期候选）3 条全部成立——P3 重叠敏感降级（sc7/sc30）+ P1 临界值未达（s7_ratio）合力，R1「供给唯一正信号」修正口径下全倒 ✅
- **守院**：fit 探索 require_fit / val 仅预注册复验（45/105 单元可算）/ seed=42 确定性 / 不落地引擎阈值红线 ✅

### 二、非阻断登记（3 项）

1. **时期映射表述**：IC"P 期仅 1 截面日，与 R1 一致"——R1 的 834 天（S3 159/S4 127/P 45）是**全段**（fit+val）统计、P1 的 452 天是 fit 段；两处同口径（state_bucket 同款，源码核对），P 期 fit 段确实仅 1 日（恐慌簇在 val 段 2026-05，R3 审计同发现）。建议表述精确为"fit 段 P 期仅 1 截面日"。
2. **S2 候选样本量**：基于 18 个去重叠截面日（原始 118 天）——样本有限但置换临界值按该时期原始样本挖（口径自洽），与 P6 DSR 呼应存在性成立。
3. **置换 null 用 pct 锚**（时期通用临界值）：符合预注册"每时期一个临界值"；逐因子 null 可另行预注册（观察项）。

### 三、裁定

- **P1 复核通过**：临界值/候选/翻转/样本防护全部核验一致；P3+P1 合力"供给候选全倒"结论成立。
- 不踩红线：审计只读；仅筛查层（registry 重标需 PM 裁定）。
- **状态**：P1 ③审计闭环（IF）；L1 顺序 P6→P3→P1→P4→P2（P4 判据 IB 待冻结、P2 判据 IE 已交付）。

## IG. P4 三档成本敏感性执行（2026-08-28，②执行，预注册 p4-cost-sensitivity-prereg-2026-08-28.md 已冻结 ID）

> ⚠️ **原编号 IF**——与③审计 IF（P1 复核，3760 行）撞号；按编号治理「先到先得、后者改号」，PM 归口改号 **IG**（2026-08-28 15:33）。

> 自主模式执行（②→PM 冻结 ID→②开跑）。产物：`data/_exp_cost_sensitivity_2026-08-28.json` + 脚本 `references/run_cost_sensitivity.py`（复用 P1 去重叠+时期临界值，三档成本 2/4/6%，runtime 12.6min）。

### 一、结果
- **三档 verdict 分布完全一致**：未评 33 / 证伪 35 / 样本不足 36 / 候选 1（三档 2%=4%=6%）；
- **唯一候选单元 sentiment|S2 三档 mean_ic 完全相同（0.16877）**；
- **翻转单元 0 / 105**（verdict(2%)=verdict(6%)）；
- **基线 317 信号**：mean_net14 15.946→15.926→15.906（2%→6% 仅 −0.04pp）、t 9.199→9.176、total 5054.97→5042.29（**−0.25%**）。

### 二、诚实登记（1 项方法学局限，③审计重点复核）
- **P4 用 IC 评估成本敏感性 = 判据设计局限**：IC 是 **Spearman 秩相关**，而成本是**均匀平移**（每品扣同量）→ **秩不变 → IC 数学上必然不变**。故「0 翻转」**部分是方法学必然，不构成「因子对成本稳健」的强证据**；
- **真正的成本影响证据来自基线收益口径**：成本翻三倍（2%→6%，+4pp）仅使基线总收益 −0.25%、t −0.02——**因基线净收益均值 15.9% 远大于成本**，实际影响确实很小（这是真结论）；
- **若需严格评估因子层成本敏感性 → 须改用收益类指标（Q5-Q1 收益差 / 净值 / SR），判据修订须重新预注册（红线）**——②建议登记此项，不擅自改判据。

### 三、结论与流向
- **成本不是 L1 结论的主要风险源**（基线证据：2%→6% 总收益仅 −0.25%）；因子层 IC 结论对成本不敏感（含方法学原因）；
- **流向**：基线成本证据 → E2 费率衔接参考；方法学局限登记 → PM/③裁定是否修订判据重跑（收益口径）；
- **P2（IC 置信区间 + MinTRL）判据已交付（IE）** 待 PM 冻结（L1 最后一项）；
- **待③审计复核**：成本换算公式、P1 临界值复用、翻转判定、方法学局限登记；
- **状态**：**P4 执行完成（IG，原 IF）**，移交③审计。

## IH. P1 闭环确认 + 撞号归口 + P4 执行确认（方法学局限裁定）+ P2 冻结（2026-08-28 15:33，PM）

### 一、P1 ③审计通过确认（IF ✅）
- S1/S2 置换临界值复现一致（偏差 <0.001）、sentiment|S2 唯一候选判定正确、三供给翻转清单成立、样本不足防护符合预注册；
- **3 项非阻断登记移交②**：①时期映射表述精确化（"fit 段 P 期仅 1 截面日"，与 R1 全段 834 天口径区分）②S2 候选样本量说明（18 个去重叠截面日 / 原始 118 天）③置换 null 用 pct 锚（观察项，逐因子 null 需另行预注册）。

### 二、编号撞号归口（IF 双号）
- 3760（③审计 P1，先）与 3783（②执行 P4，后）同用 IF → 按治理规则「先到先得、后者改号」：**②P4 条目改号 IG**（已在条目头部注明原号）；
- ③转达中「P4 判据待冻结」为信息滞后——**P4 已由 PM 于 ID（15:07）冻结并已执行完成**。

### 三、P4 执行确认（IG ✅，移交③审计）
- 三档 verdict 分布完全一致、翻转单元 0/105、基线 317 net14 三档 mean 15.946→15.906、总收益 −0.25%；
- **方法学局限（②诚实登记，PM 裁定）**：IC 为 Spearman 秩相关、成本为均匀平移 → 秩不变 → **IC 数学上必然不变**，故「0 翻转」部分属方法学必然，不构成"因子对成本稳健"的强证据；**真成本证据来自基线收益口径**（2%→6% 仅 −0.25%，基线净收益 15.9% 远大于成本 = 真结论）；
- **PM 裁定**：**不修订 P4 判据重跑**——P4 目标（成本是否 L1 主要风险源）已由基线证据回答（否）；因子层严格成本敏感性若需评估 → **另立新卡 + 新预注册（收益类指标 Q5-Q1/净值/SR）**，不在 P4 内擅自改判据（红线），②做法正确。

### 四、P2 判据冻结（IE 四要素齐备，L1 最后一项）
- **IC 95% CI（bootstrap）**：n_iter=1000 seed=42 有放回 → 2.5%/97.5% 分位；口径复用 P1 去重叠截面日 + **P4 最终成本档**（成本敏感单元取 6% 最严档）；
- **判定**：不跨 0 且不跨临界值 → 稳健；跨 0 → 不可靠；跨临界值 → 边界；
- **MinTRL**：与 P6 同款公式（年化 365/14、SR_target=1.0、γ=0.5772），每单元报告所需最少样本期数 + 对比 B 通道（提示项）；
- **守院**：fit 探索 / val 仅预注册复验 / seed=42 确定性；
- **冻结：②执行 P2**（产物 `_exp_ic_ci_2026-08-28.json`，全局 + 105 条件单元的 CI + MinTRL + 不确定性画像）。

### 五、状态
- **L1**：P6 ✅ → P3 ✅ → P1 ✅（IF 审计闭环）→ **P4 ✅（IG，待③审计）** → **P2 🔄（已冻结，执行中）**；
- registry 重标待②执行（ID 裁定）；L1 完成后 P5（单因子≠融合）跨层移交 L2/L3。

## II. P4 成本敏感性 · ③独立复核 + registry 重标核验（2026-08-28，③审计，通过 + 3 项非阻断登记）

### 一、方法学局限登记 —— 恰当 ✅（独立验证）
- **秩不变恒等独立验证**：spearman(x, y−δ) ≡ spearman(x, y)（随机样例 5 组 δ 全等，含 None 过滤变体）——②登记方向完全正确；
- **三档 IC 全等断言**：105 单元 × 三档 mean_ic/n_ics/verdict/roll 全等 mismatch=0 → **「0 翻转」为 100% 数学必然**（非②所写"部分"）——flip=0 不含任何"因子对成本稳健"信息，②诚实标注正确，建议措辞精确化；
- **基线收益口径独立复现**：317 信号三档 mean 15.9463→15.9263→15.9063 / t 9.199→9.187→9.176 / total −0.25%——完全一致；**成本不是 L1 主要风险源 = 真结论成立**。

### 二、registry 重标核验（commit 7ccaf7b）——通过 ✅
- sc7/sc30 → 重叠敏感（P3 降级）、s7_ratio → 条件校准未达（P1）、sentiment → 候选·无增量——四因子与 ID 裁定一致，cs_note 依据（P3/P1/P6）齐全；
- 21 因子分布：证伪14/重叠敏感2/条件校准未达1/候选·无增量1/存档3 = 21 闭合；重标基于 IC 口径 → **不受 P4 秩不变局限影响，无需返工**；未碰引擎、未立落地卡。

### 三、修订判据裁定（独立技术意见，与 IH 一致）——不修订、不重跑
- 判据原始风险问句已答：因子层判定指标 IC 不含成本（无关）+ 基线层 3 倍成本仅 −0.25%（稳健）；
- 收益口径画像边际价值低：registry 无活跃候选（无排序决策对象）；P2 同 IC 口径不受影响；
- 前瞻建议：未来新候选预注册采用「条件 IC（存在性）+ 收益类指标（净 alpha）」双口径，避免 P4 同类空转。

### 四、非阻断登记（3 项）
1. IG 状态行编号残留（「P4 执行完成（IF）」旧号未同步改 IG）；
2. 「0 翻转」为 100% 数学必然，②措辞"部分"建议精确化；
3. P4 因子层成本敏感性不可观测（数学恒等）——引用时勿以"0 翻转"作成本稳健论据。

### 五、状态
- **P4 ③审计闭环（II）**；L1：P6/P3/P1/P4 全闭环 → P2（IC CI + MinTRL）执行中，③无阻塞；
- 审计产物：`references/audit-p4-cost-sensitivity-2026-08-28.md` + `references/_audit_p4_recompute.py` + `data/_audit_p4_recompute_2026-08-28.json`；守院合规、审计只读。

## IJ. P2 IC 置信区间 + MinTRL 执行（2026-08-28，②执行，预注册 p2-ic-ci-prereg-2026-08-28.md 已冻结 IH）

> 自主模式执行（②→PM 冻结 IH→②开跑）。II（③P4 复核通过 + registry 重标核验通过 + 3 项非阻断）已知悉：①IG 状态行旧号已归口（P4 执行条目 = IG）；②"0 翻转"措辞精确化 = 100% 数学必然（秩不变恒等，II 独立验证）——P4 条目已按此理解，引用边界遵守（不以 0 翻转作成本稳健论据）；③registry 重标不受 P4 局限影响、无需返工 ✅。

### 执行（判据 §1-§5）
- **IC 95% CI（bootstrap）**：n_iter=1000 seed=42 有放回 → 2.5%/97.5% 分位；口径复用 P1 去重叠截面日 + **2% 成本档**（P4 确认 0 成本敏感单元）；
- **判定**：CI 跨 0 → 不可靠；不跨 0 且不跨时期临界值 → 稳健；跨临界值 → 边界；
- **MinTRL**（Bailey 2018 同款：年化 365/14、SR_target=1.0、γ=0.5772）：Q5 档收益序列 → 最少样本期数，对比现有样本与 B 通道；
- **对象**：21 因子全局 + 105 条件单元；val 仅预注册声明复验；
- **产物**：`data/_exp_ic_ci_2026-08-28.json`（全局 + 105 单元 CI + MinTRL + 不确定性画像）。
- **状态**：**P2 执行中（IJ）**，完成后移交③审计（L1 最后一项）。

## IK. P2 IC 置信区间 + MinTRL 执行完成（2026-08-28，②执行，预注册 p2-ic-ci-prereg-2026-08-28.md 已冻结 IH，启动登记 IJ）

> 自主模式执行（②→PM 冻结 IH→②开跑）。产物：`data/_exp_ic_ci_2026-08-28.json` + 脚本 `references/run_ic_ci.py`（bootstrap n_iter=1000 seed=42；复用 P1 去重叠截面日 102 + P4 成本档 2% 确认无敏感单元；runtime 764s）。

### 一、结果（fit 段，CI 2.5%/97.5% 分位）
**全局 21 因子**：稳健（负向）12 / 稳健（正向）3 / 不可靠（跨 0）3 / 样本不足（mchg 截面常数）3
| 因子 | mean_ic | CI | verdict | MinTRL |
|---|---|---|---|---|
| sentiment | +0.157 | 0.125~0.192 | 稳健（正向） | 13.9 月 |
| s7_ratio | +0.043 | 0.016~0.069 | 稳健（正向） | 15.8 月 |
| sc30 | +0.024 | 0.001~0.051 | 稳健（正向） | 144.6 月（样本极不足） |
| sc7 / th_anomaly / bid | 弱 | 跨 0 | 不可靠 | — |
| pct/z/chg/vol/th 其余/spread | 负 | 不跨 0 | 稳健（负向） | 负 SR 无 MinTRL 意义 |

**105 条件单元**：跨 0 = 42 / 未达临界 = 14 / 样本不足 = 14 / **边界 = 2** / 未评 = 33；
- ⚠️ **P1 唯一候选 `sentiment|S2` 在 CI 口径下降级为「边界（区间跨临界值）」**：CI 0.088~0.244 跨时期临界值 0.132（P1 点估计 0.169 看似过线，但 CI 下界 0.088 < 临界值）——**P2 后 L1 无任何条件候选**；
- MinTRL：sentiment|S2 需 12.7 月（现有 n=18 期 ≈4.2 月，**不足**；对照 B 通道 ~2027-04）。

### 二、执行缺陷修复记录（诚实登记）
- 首轮产物缺**全局 21 因子 MinTRL**（判据 §2.2 要求全局+条件单元；脚本只算条件单元）→ 修复脚本补算后重跑（12m44s，确定性与首轮一致）；修复只增字段不影响 verdict。

### 三、结论与流向
- **L1 可靠性工程全闭环**（P6✅③审计 / P3✅ / P1✅ / P4✅③审计 II / **P2 本条目**）：
  - 全局仅 sentiment/sc30/s7_ratio CI 稳健正向；**条件口径无任何候选**（sentiment|S2 边界）；
  - R1「无新候选」在全部 P1-P6 修正口径下**全面强化**（三供给候选 sc7/sc30/s7_ratio：P3 降级 + P1 未达 + P2 CI 窄验证）；
  - MinTRL 提示：sentiment/s7_ratio 需 1.1~1.3 年样本（现 fit 段约 2.8 年去重叠 102 日 ≈4 个月等效独立样本——**B 通道积累仍是关键**）；
- **流向**：不确定性画像 → registry 标注（P2 后 sc30/s7_ratio 全局稳健但无时期候选，registry 维持现状态，待 PM/③裁定是否补标注）；MinTRL → J-2 B 通道样本规划参考；**P5（单因子≠融合）移交 L2/L3**；
- **待③审计复核**：bootstrap 实现、分位数口径、MinTRL 公式与年化、全局修复、seed 一致性、val 复验 45/105；
- **状态**：**P2 执行完成（IK），移交③审计；L1 可靠性工程全部完成**。

## IL. P2 执行确认 + L1 收官确认 + P5 跨层移交（2026-08-28 19:10，PM）

### 一、P2 执行核验（IK ✅，移交③审计）
- **全局 21 因子**：稳健（正向）仅 sentiment（CI 0.125~0.192）/ s7_ratio / sc30（**MinTRL 144.6 月 = 样本极不足**）；稳健（负向）12 / 不可靠（跨 0）3 / 样本不足（mchg 截面常数）3；
- **P1 唯一候选 sentiment|S2 在 CI 口径下降级「边界」**：CI 0.088~0.244 跨时期临界值 0.132（P1 点估计 0.169 看似过线，CI 下界 0.088 < 临界值）→ **P2 后 L1 无任何条件候选**；
- **MinTRL**：sentiment 需 13.9 月（B 通道 ~2027-04 积累关键）；sentiment|S2 需 12.7 月（现有 n=18 ≈4.2 月等效独立样本，不足）；
- 执行缺陷诚实登记（全局 MinTRL 首轮缺失 → 修复重跑 12m44s，确定性一致，仅增字段不影响 verdict）；
- ③审计复核点：bootstrap 实现 / CI 分位口径 / MinTRL 公式与年化 / 全局修复 / seed 一致性 / val 复验 45/105。

### 二、L1 收官确认（P6→P3→P1→P4→P2 全执行完，P2 待③审计）
- **六环成果链**：P6 多重检验（引擎 DSR=1.0 非噪声；21 因子仅 sentiment/th_structure 校正后成立）→ P3 重叠修正（R1 t 值虚高；sc7/sc30 降级）→ P1 条件校准（唯一候选 sentiment|S2）→ P4 成本（非主要风险源，基线 −0.25%）→ **P2 CI（sentiment|S2 边界降级 → L1 无任何条件候选）**；
- **R1「无新候选」在 P1-P6 修正口径下全面强化**——现有因子池经五重独立统计修正（多重检验/重叠/条件/成本/CI）后无新增可靠信号；sentiment 存在性成立（P6/P1）但增量性（R1）与条件稳健性（P2）均不足；
- registry 标注：P2 后 sc30/s7_ratio 全局稳健但无时期候选 → registry 维持现状态（II 重标后），可补注「全局稳健·无时期候选」观察项（非阻断）。

### 三、P5 跨层移交（单因子≠融合）
- **P5（单因子评估 ≠ 融合价值）按计划移交 L2/L3 讨论**——融合价值检验（增量组合检验）属上层（L2/L3），L1 结论（含修正口径、临界值、CI/MinTRL）作输入；待 L2/L3 立项后单独推进。

### 四、状态
- **L1 可靠性工程全部执行完成**（P2 待③审计收官）；③审计 P2 通过后 **L1 正式闭环**；P5 移交 L2/L3；registry 补注观察项（非阻断）。
