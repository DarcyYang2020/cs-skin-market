...
8. **追加裁定（X 补）**：① 控制回撤目标包含**尽量减少黑天鹅回撤的影响**——黑天鹅不预测，但要做**事件响应式减仓**（大盘单日/3 日急跌触发组合降杠杆；持仓供给扩张全止损等现有矩阵继续强化）与**统计口径事件窗处理**（事件影响期样本标注、族特征卡事件窗分层），保证数据精确性；② **模拟盘全自动自主执行**——buy 信号自己加入、止盈/止损/到期信号自己卖出，全程无人介入（方案 0 节已补）。
