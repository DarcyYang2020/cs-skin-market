# -*- coding: utf-8 -*-
"""DECISION-6 dry-run: v2-T6 engine replay against data/replay_hybrid.db.

Read-only with respect to the official product: writes to
data/_exp_v2t6_hybrid_replay.json, never touches data/item_backtest_full_2025.json.
"""
import os, sys, json
...
os.environ["CS_MODEL_DB"] = os.environ.get("CS_MODEL_DB") or str(ROOT / "data" / "replay_hybrid.db")
...
def main():
    ...
    out = {"args": {"start": START, "end": END, "warmup": WARMUP,
                    ...
                    "engine": "v2-T6 (DECISION-6 dry-run)"},
           "generated": datetime.now().strftime("%Y-%m-%d %H:%M"),
           "aggregate": agg, "per_item": rows, "signals": sigs_out}
    with open(ROOT / "data" / os.environ.get("V2T6_OUT", "_exp_v2t6_hybrid_replay.json"), "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=1)
    print("signals:", len(sigs_out), "| saved", os.environ.get("V2T6_OUT", "data/_exp_v2t6_hybrid_replay.json"), flush=True)
