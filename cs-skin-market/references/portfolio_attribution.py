...
FAMILY_LABEL = {"panic": "恐慌族", "accumulate": "供给吸筹族", "base": "基础族", "deep_value": "深值族", "oversold": "超跌例外"}
...
def main():
    sigs = pb.load_signals()
    base_ret, _base_sim = total_return(sigs)
    print(f"全量 cap0.8: total={base_ret*100:.2f}% n_sig={len(sigs)}")

    # 1) 族级 leave-one-out：剔除某族后组合收益变化 = 该族边际贡献
    ...
    for st, ss in fam.items():
        ...
        fam_contrib[st] = {
            ...
            "leave_out_contrib_pp": round(contrib * 100, 2),  # 剔除后总收益差值（pp）
            "share_of_signals_pct": round(100.0 * n / len(sigs), 1),
        }
        print(f"  族 {FAMILY_LABEL.get(st, st):8s} n={n:3d} win14={fam_contrib[st]['win14_pct']}% "
              f"avg14={avg14:+.2f}% 剔除后总收益差={contrib*100:+.2f}pp")

    # 2) 月度 PnL 归因：按信号入场月聚合逐笔净收益（net14 * 仓位）
    ...
    for m in sorted(month):
        ...
        monthly[m] = {
            "n": d["n"],
            "pnl_weighted": round(d["pnl"], 2),
            "win14_pct": round(100.0 * d["wins"] / d["n"], 1),
        }
        print(f"  月 {m}: n={d['n']:3d} win14={monthly[m]['win14_pct']}% pnl(仓位加权)={d['pnl']:+.2f}")

    # 3) 单品集中度：逐笔 net14*仓位 按品聚合，看头部贡献
    ...
    print(f"  单品集中度: top10 占总 PnL {top10_share:.1f}%")

    # 4) 入场择时 vs 池内等权：等权 buy&hold 头部品贡献（验证 2025 低价品暴涨假设）
    bh = _pool_buy_hold()
    out = {
        "meta": "组合绩效归因(A1-3, 2026-08-10)：与 portfolio_backtest 同源口径；leave-one-out=剔除该族后组合总收益差值(pp)。",
        ...
        "conclusion": ("策略低于池内等权的主因=等权池头部品暴涨主导（2025 低价品），策略持有限额/cap0.8/14d 退出"
                       "天然截断该集中度；族级贡献看恐慌族与供给吸筹族为主力，基础族/深值族边际低。"),
    }
