...
- 贴纸庄盘专项 ✅（2026-08-12，见 decision-log「贴纸庄盘专项 · 第一性原理研究 + H1 回测 + 庄盘指纹落地」+ `references/first-principles-manipulation.md`）：第一性原理确认贴纸天然适合坐庄（供给冻结/池小深度浅/Major 事件可预期），事件 = 庄家派发窗口；实证 142/144 品脉冲、尖顶崩塌、长期不复原（区间末中位 18.7%）；H1：V0 朴素尖顶负结果（3d 崩 50% 假摔，30d 中位 +19.6%）、V3 全局峰值后 30/90d 下跌 100%（-50.5%/-67.2%，事后口径）——「峰值确认」离场规则立项 W-1（walk-forward + 三件套）；阶段 A 庄盘指纹展示块已落地（零决策参数）。S-2 Major 日历择时负结果（2026-08-12，见 decision-log「S-2 Major 日历周期择时回测 · 当届贴纸无买点」）：当届贴纸上市即峰值无买点（64 品全参数网格负期望，最优 win 18.8% mean -28.7%，walk-forward 交叉全负），当届/老贴纸赛事窗口均跌——日历择时假设证伪，贴纸买点 = 庄盘选品；进攻侧立项 W-3 存量贴纸事件前置买点（待用户配置 2026 赛程 + 事件样本积累，三件套前不进决策）。产物 `data/_exp_s2_major_calendar.json`，零引擎参数。W-3 探索版（2026-08-12，产物 `data/_exp_w3_accum_continuation.json`）：用户确认贴纸与 Major 脱钩，进攻侧重定义为吸筹形态检测——信号 A 吸筹（深回撤>70% + CV≤5% + 企稳）66 事件/43 品、39 前视 win30 69.2% / med +18.2%（同期 cv≤5% 56.7%/+4.4%，置换 p_med 0.002，候选）；信号 B 拉升承接证伪（90d 在售三组全负无分化 + B2 横盘中继 win14 17.4%——贴纸无二波）。正式 A2 待样本积累，零引擎参数。模块收尾（2026-08-12）：S-4 A2 注册假设修正（庄盘吸筹 + regime 分层）、S-5 EVENT_CALENDAR 补 2025 双 Major 历史事件、D5 同届分化入庄盘指纹块（G2 2024 上海 ratio 6.3 / EG 2021 ratio 41.1）、S-3 关闭（Major 脱钩 + W-1 证伪）、S-1 短名 2 品重命名修复；贴纸模块剩「数据积累」项：W-3 吸筹 A2（n 积累）、supply 在售分层、H4 大户交叉、D1/D2 历史在售量。W-1 峰值确认离场（2026-08-12，产物 data/_exp_w1_peak_exit.json）：confirm 7/14/21d 全时段弱有效（down30 56.6%/-4.4% vs 无确认 55.0%/-3.3%），walk-forward 前段 +15.2%（踏空）vs 后段 -8.3%——regime 依赖，固定参数不落地。W-2 深历史在售量评估（2026-08-12）：fetch_simple_kline 无在售量、chart 含 num_data 但回填 4-6h+限流风险，历史段无法事后验证 → 不批量补采，量确认走 90d 增量探索。吸筹跟踪上线并挂每日任务（references/w3_signal_tracker.py → run_daily_collect.py，66 信号：39 落地 win30 69.2% / 27 pending ETA 8/14 起 ph 实时推进）。**alpha 口径结论（2026-08-12）**：扣同期市场基准后吸筹 alpha med +8.6%、82%>0（crash regime +7.4% / calm +9.3%）——吸筹是相对市场的选品 alpha，绝对收益依赖市场环境（暴跌期禁买），正式 A2 含 regime 分层。双榜独立刷新（2026-08-12，见 decision-log「Top10 高分饰品·双榜独立刷新」）：Top10 刷新拆「综合榜/贴纸榜」两入口，scan-all 新增 scope（all/skin/sticker），scope 刷新按 name 合并回 cache（另一榜保留）且不覆盖当天 discover_history 快照——展示层零决策参数。
...
- 期望徽章抽离为外部常驻卡片 ✅（2026-08-12，纯展示层，见 decision-log「期望徽章抽离」）：族级/市场级期望信息（全局族 + B-1 regime 分层）从单品报告移除（同族品重复冗余），改由仪表盘「🧪 当前市场状态 × 信号族期望」卡片统一展示（`market_expectancy_card`：当前状态桶 × 三族当前桶分层 + 全局期望，n<5 标不足）；单品报告保留状态桶徽章与决策链 trace；`_expectancy_badge` 删除、`_fd_display` 精简为仅 trace。冒烟 104/0/0，pyflakes 0，零决策参数。

- 代码/文档/数据卫生整理 ✅（2026-08-13，非决策参数）：主要见 decision-log 同日条目；同步回放 317 信号与基准数据、测试 104 用例、新模块、in_sale NULL 口径、runtime 日志保留 365 天。
