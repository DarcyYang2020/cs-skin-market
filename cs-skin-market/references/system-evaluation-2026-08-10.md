...
**仍待做（分期大项/数据依赖）**：C-1 第二批（discover/scan 任务块按域拆分 + f-string→Jinja 统一；第一批渲染纯函数已切 `webapp/render_html.py`）、D-1（报告分层，分期；决策条溯源/术语已折叠，深层分层低优先）、F-2（求购数据 90+ 天）、A1-4（滑点校准，executions≥20）、G-2 加签后 webhook 轮换待用户配置。
