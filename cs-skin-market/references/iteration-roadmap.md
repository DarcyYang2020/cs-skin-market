...
- **依赖**：无（与研究层 R1 接入配合）。**负责**：④运维 + ②研究。**状态**：✅ ③审计通过（2026-08-27；**oos_guard 零研究脚本消费 → 等②Wave2 研究入口 R1 接 require_fit 落地，登记 DJ 四项②，接线后③复核**）。
...
- **依赖**：S1（悠悠数据可选增强，非硬前置）。**负责**：④运维 + 研发。**状态**：待执行。
...
- **依赖**：S2（台账）、O2（kill switch 暂停）。**负责**：④运维 + 研发。**状态**：待执行。
