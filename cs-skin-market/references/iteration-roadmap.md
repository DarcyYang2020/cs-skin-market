...
- **依赖**：D6（oos_zone）、R1（因子基础）。**负责**：②研究。**状态**：✅ **判据已冻结（2026-08-27 09:46，PM 裁定 DR，references/r3-family-isolation-prereg-2026-08-27.md）**：②据以开跑 6 族隔离评估（产物 `data/_exp_family_isolation_2026-08-27.json`，仅评估结论不改引擎）；红线：判据跑前定死/禁看结果调阈值/oos_zone 守院/不立落地卡。
...
- **依赖**：D2（点差派生）、R1。**负责**：②研究。**状态**：✅ **判据已冻结（2026-08-27 09:46，PM 裁定 DR，references/r5-emotion-v0-prereg-2026-08-27.md）**：②据以执行 v0 情绪评估（产物 `data/_exp_emotion_v0_2026-08-27.json`）；红线：固定权重禁优化/增量IC硬判据/oos_zone 守院/仅加分过滤不进主干。
...
- **依赖**：无。**负责**：④运维（预研）。**状态**：✅ 研发落地（2026-08-27：在售列表可采实证 / 求购成交待登录验证，报告 references/yyyp-feasibility-2026-08-27.md，待③审计）。
...
- **依赖**：S1（悠悠数据可选增强，非硬前置）。**负责**：④运维 + 研发。**状态**：✅ 研发落地（2026-08-27：交易域 orders/fills + 费率买0/卖1 + 无货拒单 + 修复既有现金 bug，冒烟 144/0/0，待③审计）。
...
- **依赖**：S2（台账）、O2（kill switch 暂停）。**负责**：④运维 + 研发。**状态**：✅ 研发落地（2026-08-27：意向单+钉钉卡片+回报入口+超时提示接线 O1，冒烟 144/0/0，待③审计）。
...
- **依赖**：D6（oos_zone/幸存者偏差）、E2（费率）。**负责**：②研究 + 研发。**状态**：待执行（待③审计口径）。
...
- **依赖**：E1（质量标签）、R1/R2（因子数据）、决策层现有。**负责**：研发（前端）+ ②研究。**状态**：待执行。
...
- **依赖**：E1/E3、R3（隔离评估）。**负责**：研发 + ②研究。**状态**：待执行。
...
- **W7-1 内生情绪 v1**（bid/spread/turnover + steamdt 成交额，待采集可行性预研 + 积累 3-6 个月）—— 依赖 S1/D2。
- **W7-2 steamdt 成交额组件**（市场级活跃度，待 S1 可行性预研 + 历史深度）。
- **W7-3 多策略形态**（策略隔离评估 R3 后定）。
