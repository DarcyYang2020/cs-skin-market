...
| 16 | 第一性分析（2026-08-08）：executions 表/API 已建但仅 1 条手动记录，执行环摩擦大、反馈不可见 | **F-1** 报告页「按建议记录执行」一键入口 | P0（第一性） | 无（表/API 现成） | 待做 |
| 16 | 同上 | **F-2** 组合仪表盘执行复盘对照卡（真实 pnl/滑点/胜率 vs 纸面） | P0（第一性） | 无（数据现成） | 待做 |
...
- **P0-P2 第一性原理落地（2026-08-08 立项）**：P0 F-1 一键执行 + F-2 执行复盘对照卡（零依赖，可立即开工）→ F-3 全市场扩池（数据积累）；
...
- F-1 一键执行录入 / F-2 执行复盘对照卡：**待做（2026-08-08 立项，P0 第一性）**——executions 表/API/自动结算已存在（2026-08-04~06），缺口是录入摩擦与收益对照不可见。
