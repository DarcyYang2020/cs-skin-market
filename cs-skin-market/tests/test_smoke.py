...
def t_paper_trading():
    ...
    try:
        ...
        # 供给扩张全止损（sc30>5）
        closed = _pt.settle_exits(m, {1: 110.0}, {1: 8.0})
        assert len(closed) == 1 and closed[0]["reason"] == "供给扩张全止损", closed
    finally:
...
def t_market_expectancy_card():
    ...
    assert "低位低估品" in card.get("fire_note", {}).get("active", ""), card  # 2026-08-18 可买类型主行
