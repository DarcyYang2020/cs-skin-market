...
反过拟合纪律（2026-08-16 用户明令，长期有效）：参数/阈值/持有期/仓位不得在同一回放样本上反复调参直至通过三关；样本内选择只能产出候选，落地必须样本外证据（B 通道约 2027-04 重验）或真实小仓 pilot（C 通道熔断）；变体实验先预注册判据再跑，正负结果一律登记（反面案例 v6c，见 decision-log 2026-08-16 N4）。
